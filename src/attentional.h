...
template <class Builder> struct AttentionalModel {
    ...
    Expression BuildGraph(const std::vector<int>& source,
        const std::vector<int>& target, ComputationGraph& cg,
        ModelStats& tstats, Expression* alignment = 0,
        const std::vector<int>* ctx = 0, Expression* coverage = 0,
        Expression* fertility = 0);
    void BuildGraph(const std::vector<int>& source,
        const std::vector<int>& target, ComputationGraph& cg,
        std::vector<std::vector<float> >& v_preds, bool with_softmax = true);
    Expression BuildGraph_Batch(const std::vector<std::vector<int> >& sources,
        const std::vector<std::vector<int> >& targets, ComputationGraph& cg,
        ModelStats& tstats, Expression* coverage = 0,
        Expression* fertility = 0); // for supporting mini-batch training
    Expression Forward(const std::vector<int>& sent, int t, bool log_prob,
        RNNPointer& prev_state, RNNPointer& state, dynet::ComputationGraph& cg,
        std::vector<Expression>& align_out);

    //---------------------------------------------------------------------------------------------
    // Build the relaxation optimization graph for the given sentence including
    // returned loss
    // (Hoang et al., 2017; https://arxiv.org/abs/1701.02854)
    void StartNewInstance(size_t algo, std::vector<dynet::Parameter>& v_params,
    ...
    void StartNewInstance(size_t algo,
        ...
        ,
        ComputationGraph& cg);
    Expression AddInput(const Expression& i_ewe_t, int t, ComputationGraph& cg,
    ...
    Expression BuildRelOptGraph(size_t algo,
        ...
        ,
        Expression* fertility = 0 /*global fertility model*/);
    Expression BuildRevRelOptGraph(size_t algo,
        std::vector<dynet::Parameter>& v_params /*source*/
        ,
        const std::vector<int>& target, dynet::ComputationGraph& cg, Dict& sd,
    ...
    std::string GetRelOptOutput(dynet::ComputationGraph& cg,
        const std::vector<dynet::Parameter>& v_relopt_params, size_t algo,
        Dict& d, bool verbose = false);
    std::string GetRelOptOutput(unsigned strategy, dynet::ComputationGraph& cg,
    ...
    // enable/disable dropout for source and target RNNs following Gal et al.,
    // 2016
    void Set_Dropout(float do_enc, float do_dec);
    void Enable_Dropout();
    void Disable_Dropout();

    void Display_ASCII(const std::vector<int>& source,
        const std::vector<int>& target, ComputationGraph& cg,
        const Expression& alignment, Dict& sd, Dict& td);

    void Display_TIKZ(const std::vector<int>& source,
        const std::vector<int>& target, ComputationGraph& cg,
        const Expression& alignment, Dict& sd, Dict& td);

    void Display_Fertility(const std::vector<int>& source, Dict& sd);

    void Display_Empirical_Fertility(const std::vector<int>& source,
        const std::vector<int>& target, Dict& sd);

    std::vector<int> Greedy_Decode(const std::vector<int>& source,
        ComputationGraph& cg, Dict& tdict, const std::vector<int>* ctx = 0);

    std::vector<int> Beam_Decode(const std::vector<int>& source,
        ComputationGraph& cg, unsigned beam_width, Dict& tdict,
        const std::vector<int>* ctx = 0);

    std::vector<int> Sample(const std::vector<int>& source,
        ComputationGraph& cg, Dict& tdict, const std::vector<int>* ctx = 0);

    void Add_Global_Fertility_Params(dynet::ParameterCollection* model,
        unsigned hidden_dim, bool _rnn_src_embeddings);
    ...
    void StartNewInstance(const std::vector<int>& src, ComputationGraph& cg,
        ModelStats& tstats, const std::vector<int>* ctx = 0);
    void StartNewInstance(const std::vector<int>& src, ComputationGraph& cg,
        const std::vector<int>* ctx = 0);
    void StartNewInstance_Batch(const std::vector<std::vector<int> >& srcs,
        ComputationGraph& cg,
        ModelStats& tstats); // for supporting mini-batch training
    Expression AddInput(unsigned tgt_tok, unsigned t, ComputationGraph& cg,
        RNNPointer* prev_state = 0);
...
#define WTF(expression)                                                        \
    std::cout << #expression << " has dimensions "                             \
              << cg.nodes[expression.i]->dim << std::endl;
...
AttentionalModel<Builder>::AttentionalModel(dynet::ParameterCollection* model,
    ...
    : builder(tlayers, (_rnn_src_embeddings) ? 3 * hidden_dim : 2 * hidden_dim,
          hidden_dim, *model)
    ...
    , giza_positional(_giza_positional)
    , giza_markov(_giza_markov)
    , giza_fertility(_giza_fertility)
    ...
    , num_aux_vecs(0)
{
    // std::cerr << "Attentionalmodel(" << _src_vocab_size  << " " <<
    // _tgt_vocab_size  << " " <<  slayers << " " << tlayers << " " <<
    // hidden_dim << " " <<  align_dim  << " " <<  _rnn_src_embeddings  << " "
    // <<  _giza_positional << ":" << _giza_markov << ":" << _giza_fertility <<
    // ":" << _global_fertility << " " <<  _doc_context << ")\n";

    p_cs = (_p_cs == nullptr)
        ? model->add_lookup_parameters(src_vocab_size, { hidden_dim })
        : *_p_cs;
    p_ct = (_p_ct == nullptr)
        ? model->add_lookup_parameters(tgt_vocab_size, { hidden_dim })
        : *_p_ct;
    ...
    for (int l = 0; l < hidden_layers; ++l) {
        if (rnn_src_embeddings)
            p_Wh0.push_back(
                model->add_parameters({ hidden_dim, 2 * hidden_dim }));
        else
...
template <class Builder> void AttentionalModel<Builder>::Enable_Dropout()
{
...
template <class Builder> void AttentionalModel<Builder>::Disable_Dropout()
{
...
template <class Builder>
void AttentionalModel<Builder>::Add_Global_Fertility_Params(
    dynet::ParameterCollection* model, unsigned hidden_dim,
...
template <class Builder> AttentionalModel<Builder>::~AttentionalModel()
{
    for (auto v : aux_vecs)
        delete v;
}
...
template <class Builder>
void AttentionalModel<Builder>::StartNewInstance(const std::vector<int>& source,
    ComputationGraph& cg, const std::vector<int>* ctx)
{
    ...
    if (!rnn_src_embeddings) {
        ...
        for (unsigned i = 0; i < slen; ++i)
            source_embeddings.push_back(concatenate(
                std::vector<Expression>({ src_fwd[i], src_bwd[i] })));
    }
    ...
    if (giza_fertility || giza_markov || giza_positional) {
        ...
        if (giza_positional) {
            ...
            i_src_len
                = repeat(cg, slen, std::log(1.0 + slen), auxiliary_vector());
        }
    }
    ...
    if (doc_context && ctx != 0) {
        ...
        if (!rnn_src_embeddings) {
            ...
            for (unsigned i = 0; i + 1 < context.size(); ++i)
                ctx_embed[i]
                    = builder_src_fwd.add_input(lookup(cg, p_cs, context[i]));
        }
...
template <class Builder>
void AttentionalModel<Builder>::StartNewInstance(const std::vector<int>& source,
    ComputationGraph& cg, ModelStats& tstats, const std::vector<int>* ctx)
{
    ...
    if (!rnn_src_embeddings) {
        for (unsigned s = 0; s < slen; ++s) {
            tstats.words_src++;
            if (source[s] == kSRC_UNK)
                tstats.words_src_unk++;
            source_embeddings.push_back(lookup(cg, p_cs, source[s]));
        }
    } else {
        // run a RNN backward and forward over the source sentence
...
template <class Builder>
void AttentionalModel<Builder>::StartNewInstance_Batch(
    const std::vector<std::vector<int> >& sources, ComputationGraph& cg,
    ModelStats& tstats)
{
    // Get the max size
    ...
    if (!rnn_src_embeddings) {
        for (unsigned l = 0; l < max_len; l++) {
            for (unsigned bs = 0; bs < sources.size(); ++bs) {
                words[bs] = (l < sources[bs].size()) ? (unsigned)sources[bs][l]
                                                     : kSRC_EOS;
                if (l < sources[bs].size()) {
                    tstats.words_src++;
                    if (sources[bs][l] == kSRC_UNK)
                        tstats.words_src_unk++;
                }
            }
        ...
        for (int l = max_len - 1; l >= 0;
             --l) { // int instead of unsigned for negative value of l
            // offset by one position to the right, to catch </s> and generally
            // not duplicate the w_t already captured in src_fwd[t]
            for (unsigned bs = 0; bs < sources.size(); ++bs)
                words[bs] = ((unsigned)l < sources[bs].size())
                    ? (unsigned)sources[bs][l]
                    : kSRC_EOS;
        ...
        for (unsigned l = 0; l < max_len; ++l)
            source_embeddings.push_back(concatenate(
                std::vector<Expression>({ src_fwd[l], src_bwd[l] })));
    }
    src = concatenate_cols(source_embeddings);
    ...
    if (giza_fertility || giza_markov || giza_positional) {
        ...
        if (giza_positional) {
            ...
            i_src_len = repeat(
                cg, max_len, std::log(1.0 + max_len), auxiliary_vector());
        }
    }
...
template <class Builder>
Expression AttentionalModel<Builder>::AddInput(
    unsigned trg_tok, unsigned t, ComputationGraph& cg, RNNPointer* prev_state)
{
    // alignment input
    ...
    if (t > 0) {
        ...
        if (prev_state)
            i_h_tm1 = concatenate(
                builder.get_h(*prev_state)); // This is required for beam search
    ...
    if (giza_markov || giza_fertility || giza_positional) {
        ...
        if (giza_markov || giza_fertility) {
            if (t > 0) {
                if (giza_fertility) {
                    ...
                    auto i_asum_pm
                        = dither(cg, i_asum, 0.0f, auxiliary_vector());
                    alignment_context.push_back(i_asum_pm);
                }
                if (giza_markov) {
                    auto i_alast_pm
                        = dither(cg, aligns.back(), 0.0f, auxiliary_vector());
        ...
        if (giza_positional) {
            ...
            auto i_tgt_idx
                = repeat(cg, slen, std::log(1.0 + t), auxiliary_vector());
        ...
        if (t > 0)
            i_e_t_input = i_e_t_input + i_wah_rep;
    ...
    Expression i_alpha_t
        = softmax(i_e_t); // FIXME: consider summing to less than one?
    aligns.push_back(i_alpha_t);
    Expression i_c_t
        = src * i_alpha_t; // FIXME: effectively summing here, consider maxing?
    ...
    Expression i_tildet_t
        = tanh(affine_transform({ i_y_t, i_Q, i_c_t, i_P, i_x_t }));
...
template <class Builder>
Expression AttentionalModel<Builder>::AddInput_Batch(
    const std::vector<unsigned>& trg_words, unsigned t, ComputationGraph& cg)
{
    // alignment input
    Expression i_wah_rep;
    if (t > 0) {
...
Expression AttentionalModel<Builder>::BuildGraph(const std::vector<int>& source,
    ...
    Expression* alignment, const std::vector<int>* ctx, Expression* coverage,
    Expression* fertility)
{
    // std::cout << "source sentence length: " << source.size() << " target: "
    ...
    for (unsigned t = 0; t < tlen; ++t) {
        tstats.words_tgt++;
        if (target[t] == kTGT_UNK)
            tstats.words_tgt_unk++;
    ...
    if (coverage != nullptr || fertility != nullptr) {
        Expression i_aligns
            = (alignment != 0) ? *alignment : concatenate_cols(aligns);
        Expression i_totals = sum_cols(i_aligns);
        Expression i_total_trim = pickrange(
            i_totals, 1, slen - 1); // only care about the non-null entries

        // AM for computer vision paper (Xu K. et al, 2015) has a penalty over
        // alignment rows deviating from 1
        if (coverage != nullptr) {
        ...
        if (fertility != nullptr) {
            ...
            Expression fbias = concatenate_cols(
                std::vector<Expression>(slen, parameter(cg, p_bfhid)));
            Expression mbias = concatenate(
                std::vector<Expression>(slen, parameter(cg, p_bfmu)));
            Expression vbias = concatenate(
                std::vector<Expression>(slen, parameter(cg, p_bfvar)));
            Expression fhid
                = tanh(transpose(fbias + parameter(cg, p_Wfhid) * src));
			...
			*fertility = -sum_cols(transpose(partition + exponent));
            ...
            // Expression exponent = cdiv(-cwise_multiply(delta, delta), 2.0f *
            // var_trim);
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildGraph_Batch(
    const std::vector<std::vector<int> >& sources,
    const std::vector<std::vector<int> >& targets, ComputationGraph& cg,
    ModelStats& tstats, Expression* coverage, Expression* fertility)
{
    ...
    for (unsigned t = 0; t < tlen; ++t) {
        for (size_t bs = 0; bs < targets.size(); bs++) {
            words[bs] = (targets[bs].size() > t) ? (unsigned)targets[bs][t]
                                                 : kTGT_EOS;
            next_words[bs] = (targets[bs].size() > (t + 1))
                ? (unsigned)targets[bs][t + 1]
                : kTGT_EOS;
            if (targets[bs].size() > t) {
                tstats.words_tgt++;
                if (targets[bs][t] == kTGT_UNK)
                    tstats.words_tgt_unk++;
            }
        }
    ...
    if (coverage != nullptr || fertility != nullptr) {
        ...
        if (fertility != nullptr) {
			...
			*fertility = -sum_cols(transpose(partition + exponent));
            ...
            Expression partition = -0.5 * log(2.0f * var_trim * 3.14159265359);
            *fertility
                = sum_batches(-sum_cols(transpose(partition + exponent)));
// note that as this is the value of the normal density, the errors
// are not strictly positive
#endif
...
template <class Builder>
Expression AttentionalModel<Builder>::Forward(const std::vector<int>& sent,
    int t, bool log_prob, RNNPointer& prev_state, RNNPointer& state,
...
// returned loss
template <class Builder>
void AttentionalModel<Builder>::ComputeTrgWordEmbeddingMatrix(
...
template <class Builder>
void AttentionalModel<Builder>::ComputeSrcWordEmbeddingMatrix(
    dynet::ComputationGraph& cg)
{
...
template <class Builder>
Expression AttentionalModel<Builder>::GetWordEmbeddingVector(
    const Expression& i_y)
{
    // expected embedding
    return (i_We /*hidden_dim x VOCAB_SIZE*/
        * i_y /*VOCAB_SIZE x 1*/); // hidden_dim x 1
}

template <class Builder>
Expression AttentionalModel<Builder>::AddInput(const Expression& i_ewe_t, int t,
    ComputationGraph& cg, RNNPointer* prev_state)
{
    Expression i_wah_rep;
    if (t > 0) {
        ...
        i_wah_rep = concatenate_cols(
            std::vector<Expression>(slen, i_wah)); // want numpy style
                                                   // broadcasting, but have to
                                                   // do this manually
    }
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildRelOptGraph(size_t algo,
    std::vector<dynet::Parameter>& v_params, dynet::ComputationGraph& cg,
    Dict& d, bool reverse, Expression* entropy, Expression* alignment,
    Expression* coverage, float coverage_C, Expression* fertility)
{
    int tlen
        = v_params
              .size(); // desired target length (excluding BOS and EOS tokens)
    ...
    for (auto t : boost::irange(0, tlen)) {
        ...
        } else if (algo == RELOPT_ALGO::EG
            || algo == RELOPT_ALGO::AEG) { // EG or AEG approach
            Expression i_p = parameter(cg, v_params[ct]);
    ...
    for (auto t : boost::irange(0, tlen + 1)) {
        ...
        Expression i_r_t
            = AddInput(i_wes[t] /*expected word embedding*/, t, cg);
        ...
        if (t >= tlen) { // for predicting EOS
            ...
            i_cost = -transpose(i_y) * log(i_softmax); // FIXME: use
                                                       // log_softmax(i_r_t)
                                                       // instead, faster?

            // add entropy regularizer to SOFTMAX or SPARSEMAX cost function
            // if (algo == RELOPT_ALGO::SOFTMAX || algo ==
            // RELOPT_ALGO::SPARSEMAX){
    ...
    if (coverage != nullptr || fertility != nullptr) {
        ...
        if (coverage != nullptr) {
            // Expression i_ones = repeat(cg, slen-2, 1.0f, auxiliary_vector());
            Expression i_ones
                = repeat(cg, slen - 2, coverage_C, auxiliary_vector());
            Expression i_penalty = squared_distance(i_total_trim, i_ones);
            *coverage = i_penalty;
        }
        ...
        if (fertility != nullptr) {
            ...
            Expression fbias = concatenate_cols(
                std::vector<Expression>(slen, const_parameter(cg, p_bfhid)));
            Expression mbias = concatenate(
                std::vector<Expression>(slen, const_parameter(cg, p_bfmu)));
            Expression vbias = concatenate(
                std::vector<Expression>(slen, const_parameter(cg, p_bfvar)));
            Expression fhid
                = tanh(transpose(fbias + const_parameter(cg, p_Wfhid) * src));
...
Expression AttentionalModel<Builder>::BuildRelOptGraph(size_t algo,
    std::vector<std::vector<dynet::Parameter> >& v_params,
    dynet::ComputationGraph& cg, Dict& d, bool reverse, Expression* entropy,
    Expression* alignment, Expression* coverage, float coverage_C,
    Expression* fertility)
{
    int ind_bos = d.convert("<s>"), ind_eos = d.convert("</s>");
    ...
    std::vector<Expression> i_wes(tlen + 1);
    i_wes[0] = concatenate_to_batch(std::vector<Expression>(v_params.size(),
        const_lookup(cg, p_ct, ind_bos))); // known BOS embedding (batched)
    ...
    for (auto t : boost::irange(0, tlen + 1)) {
        // std::cerr << "t:" << t << std::endl;
        Expression i_r_t
            = AddInput(i_wes[t] /*"batched" expected word embedding*/, t, cg);
        ...
        if (t >= tlen) { // for predicting EOS
            ...
            for (unsigned bs = 0; bs < v_params.size(); bs++) {
                if (algo == RELOPT_ALGO::SOFTMAX) { // SOFTMAX approach
                    ...
                    == RELOPT_ALGO::SPARSEMAX) { // SPARSEMAX approach
                    Expression i_p = parameter(cg, v_params[bs][ct]);
            ...
            // add entropy regularizer to SOFTMAX or SPARSEMAX cost function
            if (algo == RELOPT_ALGO::SOFTMAX
                || algo == RELOPT_ALGO::SPARSEMAX) {
                Expression i_entropy = -transpose(i_y) * log(i_y);
                v_ents.push_back(i_entropy);
            }
        }
...
void AttentionalModel<Builder>::StartNewInstance(size_t algo,
    std::vector<dynet::Parameter>& v_params, Dict& sd, ComputationGraph& cg)
{
    ...
    for (auto t : boost::irange(0, (int)v_params.size())) {
        if (algo == RELOPT_ALGO::SOFTMAX) { // SOFTMAX approach
            ...
            Expression i_p = parameter(cg, v_params[t]);
            exp_wrd_embeddings.push_back(
                GetWordEmbeddingVector(sparsemax(i_p)));
        } else if (algo == RELOPT_ALGO::EG
            || algo == RELOPT_ALGO::AEG) { // EG or AEG approach
            Expression i_p = parameter(cg, v_params[t]);
    ...
    exp_wrd_embeddings.push_back(
        const_lookup(cg, p_cs, sd.convert("</s>"))); // EOS
...
template <class Builder>
void AttentionalModel<Builder>::StartNewInstance(size_t algo,
    std::vector<std::vector<dynet::Parameter> >& v_params, Dict& sd,
    ComputationGraph& cg)
{
    std::vector<Expression> exp_wrd_embeddings;
    exp_wrd_embeddings.push_back(concatenate_to_batch(std::vector<Expression>(
        v_params.size(), const_lookup(cg, p_cs, sd.convert("<s>"))))); // BOS
    ...
    exp_wrd_embeddings.push_back(concatenate_to_batch(std::vector<Expression>(
        v_params.size(), const_lookup(cg, p_cs, sd.convert("</s>"))))); // EOS
...
template <class Builder>
Expression AttentionalModel<Builder>::BuildRevRelOptGraph(size_t algo,
    std::vector<dynet::Parameter>& v_params /*source*/
    ,
    const std::vector<int>& target, dynet::ComputationGraph& cg,
    Dict& sd /*source vocabulary*/
    ,
...
template <class Builder>
std::string AttentionalModel<Builder>::GetRelOptOutput(
    dynet::ComputationGraph& cg,
    const std::vector<dynet::Parameter>& v_relopt_params, size_t algo, Dict& d,
    bool verbose)
{
...
template <class Builder>
std::string AttentionalModel<Builder>::GetRelOptOutput(unsigned strategy,
    dynet::ComputationGraph& cg, const Sentence& i_src_sent,
    const std::vector<std::vector<dynet::Parameter> >& v_relopt_params,
    size_t algo, Dict& d, bool verbose)
{
    int ind_bos = d.convert("<s>"), ind_eos = d.convert("</s>");
    ...
    if (strategy == 0) { // minimum loss strategy
        for (unsigned k = 0; k < v_relopt_params.size(); k++) {
            ...
            for (auto& p : v_relopt_params[k]) {
                ...
                // FIXME: Add the bos/eos/unk penalties if required
                // v_y_dist[] = -1.f;// penalty since <s> never appears in the
                // middle of a target sentence.
                // v_y_dist[GetUnkId()] = -1.f;
                ...
                // ss << d.convert(index) << " ";
                // if (verbose) std::cerr << d.convert(index) << "(" << *it <<
                // ")" << " ";// console output
            }
            ...
            ModelStats stats;
            auto iloss = (1.f / (float)(i_trg.size() - 1))
                * BuildGraph(i_src_sent, i_trg, cg, stats); // normalized NLL
...
template <class Builder>
void AttentionalModel<Builder>::Display_ASCII(const std::vector<int>& source,
    const std::vector<int>& target, ComputationGraph& cg,
    const Expression& alignment, Dict& sd, Dict& td)
{
    using namespace std;
    ...
    for (unsigned i = 0; i < I; ++i) {
        ...
        int max_j = -1;
        for (unsigned j = 0; j < J; ++j) {
            float v = TensorTools::access_element(
                a, Dim({ (unsigned int)j, (unsigned int)i }));
            string symbol;
        ...
        cout << setw(20) << "max Pr=" << setprecision(3) << setw(5) << max_v
             << " @ " << max_j << endl;
    }
...
template <class Builder>
void AttentionalModel<Builder>::Display_TIKZ(const std::vector<int>& source,
    const std::vector<int>& target, ComputationGraph& cg,
    const Expression& alignment, Dict& sd, Dict& td)
{
    using namespace std;
    ...
    for (unsigned j = 0; j < J; ++j)
        cout << "\\node[anchor=west,rotate=90] at (" << j + 0.5 << ", "
             << I + 0.2 << ") { " << sd.convert(source[j]) << " };\n";
    for (unsigned i = 0; i < I; ++i)
        cout << "\\node[anchor=west] at (" << J + 0.2 << ", " << I - i - 0.5
             << ") { " << td.convert(target[i]) << " };\n";
    ...
    for (unsigned i = 0; i < I; ++i) {
        for (unsigned j = 0; j < J; ++j) {
            ...
            int val = int(v * 100);
            cout << "\\fill[blue!" << val << "!black] (" << j + eps << ", "
                 << I - i - 1 + eps << ") rectangle (" << j + 1 - eps << ","
                 << I - i - eps << ");\n";
        }
    }
...
template <class Builder>
std::vector<int> AttentionalModel<Builder>::Greedy_Decode(
    const std::vector<int>& source, ComputationGraph& cg, dynet::Dict& tdict,
...
struct Hypothesis {
    Hypothesis(){};
    Hypothesis(
        RNNPointer state, int tgt, float cst, std::vector<Expression>& al)
        : builder_state(state)
        , target({ tgt })
        , cost(cst)
        , aligns(al)
...
template <class Builder>
std::vector<int> AttentionalModel<Builder>::Beam_Decode(
    const std::vector<int>& source, ComputationGraph& cg, unsigned beam_width,
    dynet::Dict& tdict, const std::vector<int>* ctx)
{
    ...
    std::vector<unsigned> vocab(boost::copy_range<std::vector<unsigned> >(
        boost::irange(0u, tgt_vocab_size)));
    std::vector<Hypothesis> completed;

    for (unsigned steps = 0;
         completed.size() < beam_width && steps < 2 * source.size(); ++steps) {
        std::vector<Hypothesis> new_chart;

        for (auto& hprev : chart) {
            // std::cerr << "hypo t[-1]=" << tdict.convert(hprev.target.back())
            // << " cost " << hprev.cost << std::endl;
            if (giza_markov || giza_fertility)
                aligns = hprev.aligns;
            Expression i_scores = AddInput(hprev.target.back(),
                hprev.target.size() - 1, cg, &hprev.builder_state);
            Expression ydist = softmax(i_scores);

            // find the top k best next words
            auto dist = as_vector(cg.incremental_forward(ydist));
            std::partial_sort(vocab.begin(), vocab.begin() + beam_width,
                vocab.end(), [&dist](unsigned v1, unsigned v2) {
                    return dist[v1] > dist[v2];
                });

            // add to chart
            for (auto vi = vocab.begin(); vi < vocab.begin() + beam_width;
                 ++vi) {
                // std::cerr << "\t++word " << tdict.convert(*vi) << " prob " <<
                // dist[*vi] << std::endl;
                // if (new_chart.size() < beam_width) {
                Hypothesis hnew(builder.state(), *vi,
                    hprev.cost - std::log(dist[*vi]), hprev, aligns);
                if (*vi == eos_sym)
        ...
        if (new_chart.size() > beam_width) {
            ...
            std::partial_sort(new_chart.begin(), new_chart.begin() + beam_width,
                new_chart.end(), [](Hypothesis& h1, Hypothesis& h2) {
    ...
    // sort completed by score, adjusting for length -- not very effective, too
    // short!
    auto best = std::min_element(
        completed.begin(), completed.end(), [](Hypothesis& h1, Hypothesis& h2) {
...
template <class Builder>
std::vector<int> AttentionalModel<Builder>::Sample(
    const std::vector<int>& source, ComputationGraph& cg, dynet::Dict& tdict,
    const std::vector<int>* ctx)
{
    const int sos_sym = tdict.convert("<s>");
    ...
    while (target.back() != eos_sym) {
        ...
        // in rnnlm.cc there's a loop around this block -- why? can
        // incremental_forward fail?
        auto dist = as_vector(cg.incremental_forward(ydist));
        double p = rand01();
        unsigned w = 0;
        for (; w < dist.size(); ++w) {
...
template <class Builder>
void AttentionalModel<Builder>::Display_Fertility(
    const std::vector<int>& source, Dict& sd)
{
    ComputationGraph cg;
    ...
    Expression fbias = concatenate_cols(
        std::vector<Expression>(slen, parameter(cg, p_bfhid)));
    Expression mbias
        = concatenate(std::vector<Expression>(slen, parameter(cg, p_bfmu)));
    Expression vbias
        = concatenate(std::vector<Expression>(slen, parameter(cg, p_bfvar)));
    ...
    for (unsigned j = 1; j < slen - 1; ++j)
        std::cout << sd.convert(source[j]) << '\t' << mu_vec[j] << '\t'
                  << var_vec[j] << '\n';
}

template <class Builder>
void AttentionalModel<Builder>::Display_Empirical_Fertility(
    const std::vector<int>& source, const std::vector<int>& target, Dict& sd)
{
    ComputationGraph cg;
    Expression alignment;
    ModelStats stats;
