...
typedef tuple<Sentence, Sentence, int>
    SentencePair; // includes document id (optional)
...
template <class AM_t>
void TrainModel(ParameterCollection& model, AM_t& am, Corpus& training,
    Corpus& devel, Trainer& sgd, string out_file, bool curriculum,
    unsigned max_epochs, unsigned patience, unsigned lr_epochs,
    float lr_eta_decay, unsigned lr_patience, bool doco, float coverage,
    bool display, bool fert, float fert_weight);
template <class AM_t>
void TrainModel_Batch(ParameterCollection& model, AM_t& am, Corpus& training,
    Corpus& devel, Trainer& sgd, string out_file, bool curriculum,
    unsigned max_epochs, unsigned patience, unsigned lr_epochs,
    float lr_eta_decay, unsigned lr_patience, bool doco, float coverage,
    bool display, bool fert, float fert_weight);
...
template <class AM_t>
void Test_Rescore(
    ParameterCollection& model, AM_t& am, Corpus& testing, bool doco);
template <class AM_t>
void Test_Decode(ParameterCollection& model
    //, AM_t &am
    ,
    std::vector<std::shared_ptr<AM_t> >& ams, std::string test_file, bool doco,
    unsigned beam, bool r2l_target = false);
template <class AM_t>
void Test_Decode_Nbest(ParameterCollection& model
    //, AM_t &am
    ,
    std::vector<std::shared_ptr<AM_t> >& ams, string test_file, unsigned beam,
    unsigned nbest_size, bool print_nbest_scores, bool r2l_target = false);
template <class AM_t>
void Test_Kbest_Arcs(
    ParameterCollection& model, AM_t& am, string test_file, unsigned top_k);
template <class AM_t>
void Show_Fert_Stats(
    ParameterCollection& model, AM_t& am, Corpus& devel, bool global_fert);

template <class AM_t>
void LoadEnsembleModels(const string& conf_file,
...
Corpus Read_Corpus(const string& filename, bool doco,
    ...
    ,
    unsigned slen = 0, bool r2l_target = false, unsigned eos_padding = 0,
    bool swap = false);
std::vector<int> Read_Numbered_Sentence(
    const std::string& line, Dict* sd, std::vector<int>& ids);
void Read_Numbered_Sentence_Pair(const std::string& line, std::vector<int>* s,
    Dict* sd, std::vector<int>* t, Dict* td, std::vector<int>& ids);
...
int main(int argc, char** argv)
{
    ...
    if (vm.count("help") || vm.count("train") != 1
        || (vm.count("devel") != 1
               && !(vm.count("test") == 0 || vm.count("kbest") == 0
                      || vm.count("fert-stats") == 0))) {
        cout << opts << "\n";
        return 1;
    }
...
template <class rnn_t> int main_body(variables_map vm)
{
    ...
    if (DREPORT % TREPORT != 0)
        assert("dreport must be divisible by treport."); // to ensure the
                                                         // reporting on
                                                         // development data
    ...
    vector<string> train_paths
        = vm["train"].as<vector<string> >(); // to handle multiple training data
    if (train_paths.size() > 2)
        assert("Invalid -t or --train parameter. Only maximum 2 training "
               "corpora provided!");
    ...
    if (train_paths.size() == 2) // incremental training
    {
        ...
        cerr << "Reading extra training data from " << train_paths[1]
             << "...\n";
        training = Read_Corpus(train_paths[1], doco, true /*for training*/,
            vm["slen_limit"].as<unsigned>(), r2l_target & !swap,
            vm["eos_padding"].as<unsigned>());
    ...
    if (vm.count("devel")) {
        ...
        devel = Read_Corpus(vm["devel"].as<string>(), doco,
            false /*for development/testing*/, 0, r2l_target & !swap,
            vm["eos_padding"].as<unsigned>());
    }

    if (vm.count("test") && vm.count("rescore")) {
        // otherwise "test" file is assumed just to contain source language
        // strings
        cerr << "Reading test examples from " << vm["test"].as<string>()
             << endl;
        testing = Read_Corpus(vm["test"].as<string>(), doco,
            false /*for development/testing*/, 0, r2l_target & !swap, 0,
            vm.count("swap_T"));
    }
    ...
    if (vm.count("parameters"))
        ...
        ostringstream os;
        os << "am" << '_' << SLAYERS << '_' << TLAYERS << '_' << HIDDEN_DIM
           << '_' << ALIGN_DIM << '_' << flavour << "_b" << bidir << "_g"
           << (int)giza_pos << (int)giza_markov << (int)giza_fert << "_d"
           << doco << "-pid" << getpid() << ".params";
        fname = os.str();
    }
    ...
    if (!vm.count("test") && !vm.count("kbest") && !vm.count("fert-stats")) {
        ...
        if (sgd_type == 1)
            ...
            sgd = new CyclicalSGDTrainer(model, vm["lr_eta"].as<float>() / 10,
                vm["lr_eta"].as<float>(),
                (float)8 * (float)training.size() / MINIBATCH_SIZE,
                0.99994f); // FIXME: these hyperparameters are empirically set.
                           // Also see the original paper!
        else if (sgd_type == 0) // Vanilla SGD trainer
            sgd = new SimpleSGDTrainer(model, vm["lr_eta"].as<float>());
        else
            assert("Unknown SGD trainer type! (0: vanilla SGD; 1: momentum "
                   "SGD; 2: Adagrad; 3: AdaDelta; 4: Adam)");
        sgd->clip_threshold
            = vm["g_clip_threshold"].as<float>(); // * MINIBATCH_SIZE;// use
                                                  // larger gradient clipping
                                                  // threshold if training with
                                                  // mini-batching???
        sgd->sparse_updates_enabled = vm["sparse_updates"].as<bool>();
        if (!sgd->sparse_updates_enabled)
            cerr << "Sparse updates for lookup parameter(s) to be disabled!"
                 << endl;
    }
    ...
    if (vm.count("ensemble_conf")) // ensemble decoding
        ...
        pam->Set_Dropout(
            vm["dropout_enc"].as<float>(), vm["dropout_dec"].as<float>());

        bool add_fert = false;
        if (vm.count("test") || vm.count("kbest")) {
            pam->Add_Global_Fertility_Params(&model, HIDDEN_DIM,
                bidir); // testing/rescoring phase: add extra global fertility
                        // parameters (uninitialized model parameters)
            add_fert = true;
        }
        ...
        if (fert && !add_fert)
            pam->Add_Global_Fertility_Params(&model, HIDDEN_DIM,
                bidir); // training phase: add extra global fertility parameters
                        // to already-initialized model parameters

        cerr << endl
             << "Count of model parameters: " << model.parameter_count()
             << endl;

        v_ams.push_back(pam);
    }

    unsigned lr_epochs = vm["lr_epochs"].as<unsigned>(),
             lr_patience = vm["lr_patience"].as<unsigned>();
    if (lr_epochs > 0 && lr_patience > 0)
        cerr << "[WARNING] - Conflict on learning rate scheduler; use either "
                "lr_epochs or lr_patience!"
             << endl;

    if (!vm.count("test") && !vm.count("kbest") && !vm.count("fert-stats"))
        TrainModel_Batch(model, *v_ams[0], training, devel, *sgd, fname,
            vm.count("curriculum"), vm["epochs"].as<unsigned>(),
            vm["patience"].as<unsigned>(), lr_epochs,
            vm["lr_eta_decay"].as<float>(), lr_patience, doco,
            vm["coverage"].as<float>(), vm.count("display"), fert,
            vm["fert-weight"].as<float>());
    else if (vm.count("kbest"))
        Test_Kbest_Arcs(model, *v_ams[0], vm["kbest"].as<string>(),
            vm["topk"].as<unsigned>());
        ...
        if (vm.count("rescore")) { // e.g., compute perplexity scores
            ...
            if (vm["nbest_size"].as<unsigned>() > 1) {
                cerr << vm["nbest_size"].as<unsigned>() << "-best Decoding..."
                     << endl;
                Test_Decode_Nbest(model
                    //, *v_ams[0]
                    ,
                    v_ams, vm["test"].as<string>(), vm["beam"].as<unsigned>(),
                    vm["nbest_size"].as<unsigned>(),
                    vm.count("print_nbest_scores"), r2l_target);
            } else {
                cerr << "Decoding..." << endl;
                Test_Decode(model
                    //, *v_ams[0]
                    ,
                    v_ams, vm["test"].as<string>(), doco,
                    vm["beam"].as<unsigned>(), r2l_target);
            }
        }
    ...
    ParameterCollection& model, AM_t& am, Corpus& testing, bool doco)
{
    ...
    for (unsigned i = 0; i < testing.size(); ++i) {
        ...
        ComputationGraph cg;
        auto i_xent = am.BuildGraph(ssent, tsent, cg, tstats, nullptr,
            (doco) ? GetContext(testing, i) : nullptr);
    ...
    if (verbose)
        cout << "\n***TEST E=" << (tstats.loss / tstats.words_tgt)
             << " ppl=" << exp(tstats.loss / tstats.words_tgt) << ' ' << endl;

    return;
}

template <class AM_t>
// void Test_Decode(ParameterCollection &model, AM_t &am, string test_file, bool
// doco, unsigned beam, bool r2l_target)
void Test_Decode(ParameterCollection& model,
    std::vector<std::shared_ptr<AM_t> >& ams, string test_file, bool doco,
    unsigned beam, bool r2l_target)
{
    int lno = 0;

    // EnsembleDecoder<AM_t> edec(std::vector<AM_t*>({&am}), &td);//FIXME:
    // single decoder only
    ...
    while (getline(in, line)) {
        ...
        if (source.front() != kSRC_SOS && source.back() != kSRC_EOS) {
            cerr << "Sentence in " << test_file << ":" << lno
        ...
        if (beam > 0) {
            ...
            // target = am.Beam_Decode(source, cg, beam, td, (doco && num[0] ==
            // last_docid) ? &last_source : nullptr);// ensemble decoding not
            // supported yet!
    ...
    cerr << "Decoded " << lno << " sentences, completed in " << elapsed / 1000
         << "(s)" << endl;
}
...
void Test_Decode_Nbest(ParameterCollection& model
    ...
    ,
    std::vector<std::shared_ptr<AM_t> >& ams, string test_file, unsigned beam,
    unsigned nbest_size, bool print_nbest_scores, bool r2l_target)
{
    // EnsembleDecoder<AM_t> edec(std::vector<AM_t*>({&am}), &td);//FIXME:
    ...
    while (getline(in, line)) {
        ...
        float score = 0.f;
        if (verbose)
            cerr << "Executing GenerateNbest(...)... ";
        std::vector<EnsembleDecoderHypPtr> v_trg_hyps
            = edec.GenerateNbest(source, nbest_size, cg);
        if (verbose)
            cerr << "Done!" << endl;
        for (auto& trg_hyp : v_trg_hyps) {
            ...
            if (print_nbest_scores) {
                ss << " ||| "
                   << "AM=" << -score / (target.size() - 1) << " ||| "
                   << -score / (target.size() - 1); // normalize by target
                                                    // length, following Moses's
                                                    // N-best format.
            }
...
void Test_Kbest_Arcs(
    ParameterCollection& model, AM_t& am, string test_file, unsigned top_k)
{
    // only suitable for monolingual setting, of predicting a sentence given
    // preceeding sentence
    cerr << "Reading test examples from " << test_file << endl;
    unsigned lno = 0;
    ifstream in(test_file);
    ...
    last_items.push_back(SentencePair(Sentence({ kSRC_SOS, kSRC_EOS }),
        Sentence({ kTGT_SOS, kTGT_EOS }), -1));
    unsigned snum = 0;
    unsigned count = 0;
    ...
    std::vector<std::shared_ptr<ParameterCollection> >& mods)
{
    ...
    while (getline(inpf, line)) {
        ...
        // each line has the format: SLAYERS \t TLAYERS \t HIDDEN_DIM \t
        // ALIGN_DIM \t bidir \t giza_pos \t giza_markov \t giza_fert \t fert
        cerr << "Loading model " << i + 1 << "..." << endl;
        stringstream ss(line);
        unsigned sl, tl, hd, ad;
        bool bidir, giza_pos, giza_markov, giza_fert, glo_fert;
        string model_file;
        ss >> sl >> tl >> hd >> ad >> bidir >> giza_pos >> giza_markov
            >> giza_fert >> glo_fert;
        ss >> model_file;
        mods[i].reset(new ParameterCollection());
        ams[i].reset(
            new AM_t(mods[i].get(), SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, sl, tl, hd,
                ad, bidir, giza_pos, giza_markov, giza_fert, false, glo_fert));
        ams[i]->Add_Global_Fertility_Params(mods[i].get(), hd, bidir);
        Initialise(*mods[i], model_file);

        cerr << "Done!" << endl;
        cerr << "Count of model parameters: " << mods[i++]->parameter_count()
             << endl;
    }
    cerr << endl;
}
...
void TrainModel(ParameterCollection& model, AM_t& am, Corpus& training,
    Corpus& devel, Trainer& sgd, string out_file, bool curriculum,
    unsigned max_epochs, unsigned patience, unsigned lr_epochs,
    float lr_eta_decay, unsigned lr_patience, bool doco, float cov_weight,
    bool display, bool fert, float fert_weight)
{
    ...
    if (curriculum) {
        ...
        for (auto& landi : lengths) {
            for (unsigned k = i * curriculum_steps / lengths.size();
                 k < curriculum_steps; ++k)
    ...
    if (display) {
        ...
        for (unsigned i = 0; i < devel.size(); ++i) {
            ...
            ModelStats stats;
            auto i_loss = am.BuildGraph(ssent, tsent, cg, stats, &alignment,
                (doco) ? GetContext(devel, i) : nullptr);
            cg.forward(i_loss);
    ...
    while (epoch < max_epochs) {
        ...
        for (unsigned iter = 0; iter < report_every_i; ++iter) {
            if (si == training.size()) {
                ...
                // learning rate scheduler 1: after lr_epochs, for every next
                // epoch, the learning rate will be decreased by a factor of
                // eta_decay.
                ...
                if (curriculum
                    && epoch
                        < order_by_length.size()) { // for curriculum learning
                    order = order_by_length[epoch];
                    cerr << "Curriculum learning, with " << order.size()
                         << " examples\n";
                }
            ...
            if (verbose && iter + 1 == report_every_i) {
                ...
                am.Greedy_Decode(ssent, cg, td, (doco)
                        ? GetContext(training, order[si % order.size()])
                        : nullptr);
                cerr << "\nDecoding source, sampling: ";
                am.Sample(ssent, cg, td, (doco)
                        ? GetContext(training, order[si % order.size()])
                        : nullptr);
            }
            ...
            tie(ssent, tsent, docid) = training[order[si % order.size()]];
            ComputationGraph cg;
            if (DEBUGGING_FLAG) { // see
                                  // http://dynet.readthedocs.io/en/latest/debugging.html
            ...
            ModelStats ctstats;
            Expression i_xent
                = am.BuildGraph(ssent, tsent, cg, ctstats, &i_alignment,
                    (doco) ? GetContext(training, order[si % order.size()])
                           : nullptr,
            ...
            if (!is_valid(closs)) {
                std::cerr << "***Warning***: nan or -nan values occurred!"
                          << std::endl;
                continue;
            }
        ...
        cerr << "sents=" << si << " src_unks=" << tstats.words_src_unk
             << " trg_unks=" << tstats.words_tgt_unk
             << " E=" << (tstats.loss / tstats.words_tgt)
             << " ppl=" << exp(tstats.loss / tstats.words_tgt) << ' ';
        if (cov_weight > 0)
            cerr << "cov_penalty=" << cov_penalty / tstats.words_src << ' ';
        if (fert)
            cerr << "fert_ppl=" << exp(loss_fert / tstats.words_src) << ' ';
        cerr << "[time_elapsed=" << elapsed << "(msec)"
             << " (" << tstats.words_tgt * 1000 / elapsed << " words/sec)]"
             << endl;
        ...
        if (report % dev_every_i_reports == 0) {
            ...
            for (unsigned i = 0; i < devel.size(); ++i) {
                tie(ssent, tsent, docid) = devel[i];
                ComputationGraph cg;
                auto i_xent = am.BuildGraph(ssent, tsent, cg, dstats, nullptr,
                    (doco) ? GetContext(devel, i) : nullptr, nullptr, nullptr);
            ...
            if (dstats.loss < best_loss) {
                ...
                // oa << model;
                dynet::save_dynet_model(out_file, &model); // FIXME: use binary
                                                           // streaming instead
                                                           // for saving disk
                                                           // spaces
                cpt = 0;
            ...
            cerr << "----------------------------------------------------------"
                    "----------------------------------------------"
                 << endl;
            cerr << "***DEV [epoch=" << (lines / (double)training.size())
                 << " eta=" << sgd.learning_rate << "]"
                 << " sents=" << devel.size()
                 << " src_unks=" << dstats.words_src_unk
                 << " trg_unks=" << dstats.words_tgt_unk
                 << " E=" << (dstats.loss / dstats.words_tgt)
                 << " ppl=" << exp(dstats.loss / dstats.words_tgt) << " ";
            if (cpt > 0)
                cerr << "(not improved, best ppl on dev so far = "
                     << exp(best_loss / dstats.words_tgt) << ") ";
            timer_iteration.show();

            // learning rate scheduler 2: if the model has not been improved for
            // lr_patience times, decrease the learning rate by lr_eta_decay
            // factor.
            if (lr_patience > 0 && cpt > 0 && cpt % lr_patience == 0) {
                cerr << "The model has not been improved for " << lr_patience
                     << " times. Decreasing the learning rate..." << endl;
                sgd.learning_rate /= lr_eta_decay;
            }

            // another early stopping criterion
            if (patience > 0 && cpt >= patience) {
                cerr << "The model has not been improved for " << patience
                     << " times. Stopping now...!" << endl;
                cerr << "No. of epochs so far: " << epoch << "." << endl;
                cerr << "Best ppl on dev: " << exp(best_loss / dstats.words_tgt)
                     << endl;
                cerr << "------------------------------------------------------"
                        "--------------------------------------------------"
                     << endl;
...
struct DoubleLength {
    DoubleLength(const Corpus& cor_)
        : cor(cor_)
    {
    }
...
// --------------------------------------------------------------------------------------------------------------------------------
// The following codes are referenced from lamtram toolkit
...
{
    cerr << endl
         << "Creating minibatches for training data (using minibatch_size="
         << max_size << ")..." << endl;
    ...
    for (size_t i = 0; i < train_ids.size(); i++) {
        max_len = std::max(max_len, Calc_Size(std::get<0>(cor[train_ids[i]]),
                                        std::get<1>(cor[train_ids[i]])));
...
{
    if (MINIBATCH_SIZE == 1) {
        TrainModel(model, am, training, devel, sgd, out_file, curriculum,
            max_epochs, patience, lr_epochs, lr_eta_decay, lr_patience, doco,
            cov_weight, display, fert, fert_weight);
        return;
    }
    ...
    Create_MiniBatches(training, minibatch_size, train_src_minibatch,
        train_trg_minibatch, train_ids_minibatch);
    ...
    if (display) {
        ...
        for (unsigned i = 0; i < devel.size(); ++i) {
            ...
            Expression alignment;
            ModelStats stats;
            auto iloss = am.BuildGraph(ssent, tsent, cg, stats, &alignment,
                (doco) ? GetContext(devel, i) : nullptr);
    ...
    cerr << endl << "***SHUFFLE\n";
    std::shuffle(
        train_ids_minibatch.begin(), train_ids_minibatch.end(), *dynet::rndeng);
    ...
    while (epoch < max_epochs) {
        ...
        for (unsigned iter = 0; iter < dev_every_i_reports;) {
            if (id == train_ids_minibatch.size()) {
                ...
                cerr << "***SHUFFLE\n";
                std::shuffle(train_ids_minibatch.begin(),
                    train_ids_minibatch.end(), *dynet::rndeng);
            ...
            ModelStats ctstats;
            Expression i_xent = am.BuildGraph_Batch(
                train_src_minibatch[train_ids_minibatch[id]],
                train_trg_minibatch[train_ids_minibatch[id]], cg, ctstats,
            ...
            if (sid / report_every_i != last_print
                ...
                sgd.status();
                cerr << "sents=" << sid << " ";
                cerr /*<< "loss=" << tstats.loss*/ << "src_unks="
                ...
                if (fert)
                    cerr << "fert_ppl=" << exp(loss_fert / tstats.words_src)
                         << ' ';
                cerr /*<< "time_elapsed=" << elapsed*/ << "("
                                                       << tstats.words_tgt
                        * 1000 / elapsed << " words/sec)"
                                                       << endl;
        ...
        cerr << "--------------------------------------------------------------"
             ...
             << endl;
        cerr << "***DEV [epoch="
             << (float)epoch + (float)sid / (float)training.size()
             << " eta=" << sgd.learning_rate << "]"
             << " sents=" << devel.size()
             << " src_unks=" << dstats.words_src_unk
             << " trg_unks=" << dstats.words_tgt_unk
        ...
        // learning rate scheduler 2: if the model has not been improved for
        // lr_patience times, decrease the learning rate by lr_eta_decay factor.
        if (lr_patience > 0 && cpt > 0 && cpt % lr_patience == 0) {
            cerr << "The model has not been improved for " << lr_patience
                 << " times. Decreasing the learning rate..." << endl;
            sgd.learning_rate /= lr_eta_decay;
...
Corpus Read_Corpus(const string& filename, bool doco, bool cid, unsigned slen,
    bool r2l_target, unsigned eos_padding, bool swap)
{
    ...
    while (getline(in, line)) {
        ...
        if (!swap) {
            if (doco)
                Read_Numbered_Sentence_Pair(
                    line, &source, &sd, &target, &td, identifiers);
            else
                read_sentence_pair(line, source, sd, target, td);
        } else {
            if (doco)
                Read_Numbered_Sentence_Pair(
                    line, &source, &td, &target, &sd, identifiers);
            else
    ...
    if (cid)
        cerr << lc << " lines, " << stoks << " & " << ttoks
             << " tokens (s & t), " << sd.size() << " & " << td.size()
             << " types\n";
    else
        cerr << lc << " lines, " << stoks << " & " << ttoks
             << " tokens (s & t)\n";
...
std::vector<int> Read_Numbered_Sentence(
    const std::string& line, Dict* sd, vector<int>& identifiers)
{
...
void Read_Numbered_Sentence_Pair(const std::string& line, std::vector<int>* s,
    Dict* sd, std::vector<int>* t, Dict* td, vector<int>& identifiers)
{
    std::istringstream in(line);
    std::string word;
    std::string sep = "|||";
...
void Load_Vocabs(const string& src_vocab_file, const string& trg_vocab_file)
{
    ...
    string sword, tword;
    while (getline(if_src_vocab, sword))
        sd.convert(sword);
    while (getline(if_trg_vocab, tword))
        td.convert(tword);
...
void Initialise(ParameterCollection& model, const string& filename)
{
    ...
    // ia >> model;
    dynet::load_dynet_model(filename,
        &model); // FIXME: use binary streaming instead for saving disk spaces
}
