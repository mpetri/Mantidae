...
int main(int argc, char** argv)
{
    ...
    opts.add_options()("help", "print help message")("config,c",
        ...
        ("train,t", value<string>(),
            ...
            "each line consisting of source ||| target.")("devel,d",
            value<string>(),
            "file containing development sentences.")("test,T", value<string>(),
            "file containing testing sentences (for decoding).")("rescore,r",
            value<string>(), "rescore (source, target) sentence pairs")
        //-----------------------------------------
        ("parameters,p", value<string>(), "save best parameters to this file")(
            "initialise,i", value<vector<string> >(),
        ...
        //-----------------------------------------
        ("dir", value<bool>()->default_value(true),
            "enable/disable translation direction; source-to-target by "
            "default")("beam,b", value<unsigned>()->default_value(0),
            "size of beam in decoding; 0=greedy")("nbest_size",
            value<unsigned>()->default_value(1),
            "nbest size of translation generation/decoding; 1 by default")
        //-----------------------------------------
        ("treport", value<unsigned>()->default_value(50),
            "no. of training instances for reporting training status")(
            "dreport", value<unsigned>()->default_value(500),
            "no. of training instances for reporting training status on "
            "development data")
        //-----------------------------------------
        ("sgd_trainer", value<unsigned>()->default_value(0),
            ...
            value<bool>()->default_value(true), "enable/disable sparse "
                                                "update(s) for lookup "
                                                "parameter(s); true by default")
        //-----------------------------------------
        ("gru", "use Gated Recurrent Unit (GRU) for recurrent structure; "
                "default RNN")("lstm", "use Long Short Term Memory (LSTM) for "
            ...
            "dglstm", "use Depth-Gated Long Short Term Memory (DGLSTM) "
                      ...
                      "structure; default RNN") // FIXME: add this to dynet?
        //-----------------------------------------
        ("epochs,e", value<unsigned>()->default_value(20),
            "maximum number of training epochs")
        //-----------------------------------------
        ("lr_eta", value<float>()->default_value(0.01f),
            "SGD learning rate value (e.g., 0.01 for simple SGD trainer)")(
            "lr_eta_decay", value<float>()->default_value(2.0f),
            "SGD learning rate decay value")("lr_epochs",
            value<unsigned>()->default_value(0), "no. of epochs for starting "
                                                 "learning rate annealing "
                                                 "(e.g., halving)")
        //-----------------------------------------
        ("trace-weight", value<float>()->default_value(0.1f),
            "indicate weight for trace bonus term")
        //-----------------------------------------
    ...
    if (vm.count("help") || vm.count("train") != 1
        || (vm.count("test") != 1 && vm.count("devel") != 1
               && vm.count("rescore") != 1)) {
        cout << opts << "\n";
        return EXIT_FAILURE;
    }
...
template <class rnn_t> int main_body(variables_map vm)
{
    ...
    if (!vm.count("parameters")) {
        ostringstream os;
        os << "bam" << '_' << SLAYERS << '_' << TLAYERS << '_' << HIDDEN_DIM
           << '_' << ALIGN_DIM << "_lstm"
           << "_b" << BIDIR << "_g" << (int)GIZA_P << (int)GIZA_M << (int)GIZA_F
           << "-pid" << getpid() << ".params";
        fname = os.str();
    } else {
    ...
    BiAttentionalModel<rnn_t> bam(&model, BIDIR, GIZA_P, GIZA_M, GIZA_F, doco,
        FERT, vm["trace-weight"].as<float>(), vm.count("embedding_shared"));
    ...
    if (vm.count("initialise")) {
        ...
        if (init_files.size() == 1) {
            ...
            cerr << "initialising from " << init_files[0] << " and "
                 << init_files[1] << endl;
    ...
    if (vm.count("rescore")) {
        ...
        for (unsigned i = 0; i < testing.size(); ++i) {
            ComputationGraph cg;
            auto idloss
                = bam.BuildGraph(testing[i].first, testing[i].second, cg);

            dchars_s += testing[i].first.size() - 1;
            dchars_t += testing[i].second.size() - 1;
            dchars_tt
                += std::max(testing[i].first.size(), testing[i].second.size())
                - 1; // max or min?
            ...
            cout << " ||| " << (loss_s2t / (testing[i].second.size() - 1))
                 ...
                 << (loss_trace / (std::max(testing[i].first.size(),
                                       testing[i].second.size())
                                      - 1))
                 << std::endl;
        }

        cerr << endl
             << " E = " << (dloss / (dchars_s + dchars_t))
             << " ppl=" << exp(dloss / (dchars_s + dchars_t))
             << ' '; // FIXME: kind of hacky, as trace should be normalised
                     // differently
    ...
    if (vm.count("test")) {
        // use vm["dir"].as<bool>() for choosing either source-to-target or
        ...
        if (vm["dir"].as<bool>() == true)
            // pam = &bam.s2t_model;
            v_ams.push_back(std::make_shared<AttentionalModel<rnn_t> >(
                bam.s2t_model)); // FIXME: single decoder for now
        else {
            // pam = &bam.t2s_model;
            v_ams.push_back(std::make_shared<AttentionalModel<rnn_t> >(
                bam.t2s_model)); // FIXME: single decoder for now
        ...
        while (getline(in, line)) {
            ...
            if (beam > 0) {
                ...
                // target = bam.Beam_Decode(source, cg, beam, td, (doco &&
                // num[0] == last_docid) ? &last_source : nullptr);

                // Vu's beam search implementation
                EnsembleDecoderHypPtr trg_hyp
                    = edec.Generate(source, cg); // 1-best
                ...
                target = pam->Greedy_Decode(source, cg, td,
                    (doco && num[0] == last_docid) ? &last_source : nullptr);
    ...
    int report = 0;
    unsigned lines = 0;
    unsigned epoch = 0, max_epochs = vm["epochs"].as<unsigned>(),
             lr_epochs = vm["lr_epochs"].as<unsigned>();
    ...
    while (epoch < max_epochs) {
        ...
        for (unsigned i = 0; i < report_every_i; ++i) {
            if (si == training.size()) {
                ...
                if (lr_epochs > 0 && epoch >= lr_epochs)
                    sgd->learning_rate
                        /= lr_eta_decay; // @vhoang2: learning rate annealing
                                         // (after lr_epochs, for every next
                                         // epoch, the learning rate will be
            ...
            chars_tt += std::max(spair.first.size(), spair.second.size())
                - 1; // max or min?
            ++si;
            ...
            if (vm.count("verbose")
                && (i + 1) == report_every_i) { // display the last sentence
                                                // pair' alignments
                // if (si == 1) {
                // Expression aligns = concatenate({transpose(bam.src_align),
                // bam.tgt_align});
                // cerr << cg.get_value(aligns.i) << "\n";
                bam.s2t_model.Display_ASCII(
                    spair.first, spair.second, cg, bam.s2t_align, sd, td);
                bam.t2s_model.Display_ASCII(
                    spair.second, spair.first, cg, bam.t2s_align, td, sd);
                cerr << "\txent_s2t " << as_scalar(cg.get_value(bam.s2t_xent.i))
                     << "\txent_t2s " << as_scalar(cg.get_value(bam.t2s_xent.i))
                     << "\ttrace " << as_scalar(cg.get_value(bam.trace_bonus.i))
                     << endl;
            }
        }
        ...
        sgd->status();
        cerr << " E=" << (loss / (chars_s + chars_t))
             << " ppl=" << exp(loss / (chars_s + chars_t))
             << ' '; // FIXME: kind of hacky, as trace should be normalised
                     // differently
        ...
        if (report % dev_every_i_reports == 0) {
            ...
            for (auto& spair : dev) {
                ...
                dchars_tt
                    += std::max(spair.first.size(), spair.second.size()) - 1;
            }
            ...
            if (dloss < best) {
                ...
                // oa << model;
                dynet::save_dynet_model(fname, &model); // FIXME: use binary
