...
int main(int argc, char** argv)
{
    ...
    opts.add_options()("help", "print help message")("config,c",
        ...
        ("train,t", value<string>(),
            "file containing training sentences, with "
            "each line consisting of source ||| target.")("src_vocab",
        ...
        //-----------------------------------------
        ("initialise,i", value<string>(),
            "load pre-trained parameters (left-to-right AM model) from file")(
            "initialise_r2l", value<string>(),
            "load pre-trained parameters (right-to-left AM model) from file")(
            "initialise_t2s", value<string>(), "load pre-trained parameters "
                                               "(target-to-source AM model) "
                                               "from file")("initialise_bam",
            value<string>(), "load pre-trained parameters (integrated "
                             "source-to-target and target-to-source BAM model) "
                             "from file")("initialise_rnnlm", value<string>(),
            "load pre-trained parameters (RNNLM model) from file")
        //-----------------------------------------
        ...
        ("gru", "use Gated Recurrent Unit (GRU) for recurrent structure; "
                "default RNN")("lstm", "use Long Short Term Memory (GRU) for "
                                       "recurrent structure; default RNN")(
            "dglstm", "use Depth-Gated Long Short Term Memory (DGLSTM) "
                      "(Kaisheng et al., 2015; "
                      "https://arxiv.org/abs/1508.03790) for recurrent "
                      "structure; default RNN")
        //-----------------------------------------
        ...
        //-----------------------------------------
        ("relopt_algo", value<unsigned>()->default_value(RELOPT_ALGO::EG),
            "The algorithm (1:EG; 2:AdaptiveEG 3:SOFTMAX; 5:SPARSEMAX) for "
            "relaxed optimization")("relopt_aeg_opt",
            value<unsigned>()->default_value(0),
            "The algorithm option for AEG (0: Adam; 1: RMSProp; default Adam")(
            "relopt_init",
            value<unsigned>()->default_value(RELOPT_INIT::UNIFORM),
            "The initialization method (0:UNIFORM; 1:REFERENCE_PROBABILITY; "
            "2:REFERENCE_ONE_HOT) for relaxed optimization")("relopt_eta",
            value<float>()->default_value(1.f),
            "The learning rate for relaxed optimization")("relopt_eta_decay",
            value<float>()->default_value(2.f),
            "The learning rate decay for relaxed optimization")(
            "relopt_eta_power", value<float>()->default_value(0.f),
            "The learning rate power for relaxed optimization")(
            "relopt_momentum", value<float>()->default_value(0.f),
            "The momentum weight for gradient descent training")(
            "relopt_max_iters", value<unsigned>()->default_value(MAX_ALGO_STEP),
            "The maximum iteration for relaxed optimization")("relopt_m_weight",
            value<float>()->default_value(1.f),
            "The weight of main model (left-to-right/source-to-target)")(
            "relopt_coverage_weight", value<float>()->default_value(0.f),
            "The coverage penalty weight")("relopt_coverage_C",
            value<float>()->default_value(1.f),
            "The coverage C (e.g., >=1 and <=3")("relopt_glofer_weight",
            value<float>()->default_value(0.f), "The global fertility weight")(
            "relopt_jdec_bidir_alpha", value<float>()->default_value(0.f),
            "The interpolation weight for joint decoding in bidirectional "
            "models")("relopt_jdec_biling_alpha",
            value<float>()->default_value(0.f),
            "The interpolation weight for joint decoding in bilingual models")(
            "relopt_jdec_biling_trace_alpha",
            value<float>()->default_value(0.f),
            "The trace bonus weight for joint decoding in bilingual models")(
            "relopt_jdec_mlm_alpha", value<float>()->default_value(0.f),
            "The interpolation weight for joint decoding with additional "
            "language model(s)")("relopt_clr_lb_eta",
            value<float>()->default_value(0.f),
            "the lower bound of eta for EG with cyclical learning rate")(
            "relopt_clr_ub_eta", value<float>()->default_value(0.f),
            "the uppper bound of eta for EG with cyclical learning rate")(
            "relopt_clr_stepsize", value<float>()->default_value(0.f),
            "the stepsize for EG with cyclical learning rate")(
            "relopt_clr_gamma", value<float>()->default_value(0.f),
            "the gamma for EG with cyclical learning rate")("relopt_aeg_beta_1",
            value<float>()->default_value(0.9f),
            "the beta_1 hyperparameter of adaptive EG")("relopt_aeg_beta_2",
            value<float>()->default_value(0.999f),
            "the beta_2 hyperparameter of adaptive EG")("relopt_aeg_eps",
            value<float>()->default_value(1e-8),
            "the epsion hyperparameter of Adam-based EG")("relopt_ent_gamma",
            value<float>()->default_value(1.f),
            "The hyper-parameter for weighting the entropy regularizer of "
            "SOFTMAX or SPARSEMAX")("relopt_noise_eta",
            value<float>()->default_value(0.f),
            "The hyper-parameter for injecting the noise during gradient "
            "update")("relopt_noise_gamma",
            value<float>()->default_value(0.55f),
            "The hyper-parameter for injecting the noise during gradient "
            "update")("relopt_sgld", value<unsigned>()->default_value(0),
            "impose the use of SGLD methods (1: SGLD; 2: pSGLD); none by "
            "default")("relopt_add_extra_words",
            value<unsigned>()->default_value(0),
            "No. of extra words added")("src_in",
            value<string>()->default_value(""), "File to read the source from")(
            "ref_in", value<string>()->default_value(""),
            "File to read the reference from")(
            "ref_nbest", "ref_in is a K-best translation list ")("trg_out",
            value<string>()->default_value(""),
            "File to write the output files")("cline",
            value<unsigned>()->default_value(0), "line number to be continued "
                                                 "for processing (used in a "
                                                 "crashed running)")
        //-----------------------------------------
    ...
    if (vm.count("help") || vm.count("train") != 1 || vm.count("ref_in") != 1
        || vm.count("src_in") != 1) {
        cout << opts << "\n";
        return 1;
    }
...
template <class rnn_t> int main_body(variables_map vm)
{
    ...
    AttentionalModel<rnn_t> am(&model, SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, SLAYERS,
        TLAYERS, HIDDEN_DIM, ALIGN_DIM, bidir, giza_pos, giza_markov, giza_fert,
        doco, fert);

    if (fert)
        am.Add_Global_Fertility_Params(&model, HIDDEN_DIM,
    ...
    if (vm.count("initialise")) {
        ...
        cerr << "Count of model parameters: " << model.parameter_count()
             << endl;
    } else
        assert("No pre-trained model loaded! Failed!");

    // right-to-left and source-to-target model
    AttentionalModel<rnn_t>* p_am_r2l = nullptr; // FIXME: relax the use of
                                                 // pointer here (see
                                                 // RNNLanguageModel)?
    if (vm.count("initialise_r2l")) {
        cerr << "*Loading right-to-left AM model..." << endl;
        p_am_r2l = new AttentionalModel<rnn_t>(&model_l2r, SRC_VOCAB_SIZE,
        ...
        cerr << "Count of model parameters: " << model_l2r.parameter_count()
             << endl;
    }

    // target-to-source model
    AttentionalModel<rnn_t>* p_am_t2s = nullptr; // FIXME: relax the use of
                                                 // pointer here (see
                                                 // RNNLanguageModel)?
    if (vm.count("initialise_t2s")) {
        cerr << "*Loading target-to-source AM model..." << endl;
        p_am_t2s = new AttentionalModel<rnn_t>(&model_t2s, TGT_VOCAB_SIZE,
            SRC_VOCAB_SIZE, SLAYERS, TLAYERS, HIDDEN_DIM, ALIGN_DIM, bidir,
            giza_pos, giza_markov, giza_fert, doco, fert);
        Initialise(model_t2s, vm["initialise_t2s"].as<string>());
        cerr << "Count of model parameters: " << model_t2s.parameter_count()
             << endl;
    }
    ...
    if (vm.count("initialise_bam")) {
        cerr << "*Loading biattentional (integrated source-to-target and "
                "target-to-source) BAM model..."
             << endl;
        p_bam = new BiAttentionalModel<rnn_t>(&model_bam, bidir, giza_pos,
            giza_markov, giza_fert, doco, fert,
            vm["relopt_jdec_biling_trace_alpha"].as<float>());
        Initialise(model_bam, vm["initialise_bam"].as<string>());
        cerr << "Count of model parameters: " << model_bam.parameter_count()
             << endl;
    }
    ...
    if (vm.count("initialise_rnnlm")) {
        ...
        cerr << "Count of model parameters: " << model_mlm.parameter_count()
             << endl;
    }
    ...
    RelOptDecoder<AttentionalModel<rnn_t>, BiAttentionalModel<rnn_t>,
        RNNLanguageModel<rnn_t> >
        relopt_decoder;
    ...
    relopt_cf.jdec_biling_trace_alpha
        = vm["relopt_jdec_biling_trace_alpha"].as<float>();
    ...
    relopt_decoder.LoadModel(&am, p_am_r2l, p_am_t2s, p_bam, &rnnlm, &sd, &td);
    cerr << " done!" << endl << endl;
    ...
    if (!*src_in)
        assert(
            "Could not find src_in file "); // << vm["src_in"].as<std::string>()
                                            // << "/" <<
                                            // vm["ref_in"].as<std::string>());
    ...
    if (!*trg_out && !*costs_out)
        assert(
            "Could not find trg_out files "); // <<
                                              // vm["trg_out"].as<std::string>());

    // Get the references (e.g., greedy Viterbi, beam search, (human) reference)
    if (!vm.count("ref_nbest")) {
        std::vector<std::string> strs;
        boost::split(
            strs, vm["ref_in"].as<std::string>(), boost::is_any_of("|"));
        ...
        vector<tuple<float, float, float> > avg_scores(
            refs_in.size(), make_tuple(0.f, 0.f, 0.f));
        ...
        while (getline(*src_in, line_src)) {
            ...
            if (verbose)
                cerr << "Decoding sentence: " << line_src << endl;
            unsigned j = 0;
            for (auto& ref : lines_ref) {
                ...
                if (verbose) {
                    ...
                    cerr << "Referencing from: " << ref
                         << " (discrete cost=" << gcost_ref << ")" << endl;
                }
                ...
                if (!timing)
                    ...
                    ir = relopt_decoder.GDDecode(line_src, ref, relopt_cf,
                        dti); // with timing benchmarking

                *trg_out << ir.decoded_sent << endl; // write to output(s)
                if (verbose) {
                    gcost_inf = relopt_decoder.GetNLLCost(
                        line_src, "<s> " + ir.decoded_sent + " </s>");

                    cerr << "Inference result: " << ir.decoded_sent << endl;

                    std::get<0>(avg_scores[j])
                        += gcost_ref; // discrete cost of reference
                    std::get<1>(avg_scores[j])
                        += ir.cost; // fractional/continuous cost of inference
                                    // result
                    std::get<2>(avg_scores[j])
                        += gcost_inf; // discrete cost of inference result

                    cerr << "Reference's discrete cost=" << gcost_ref << endl;
                    cerr << "Inference result's continuous cost=" << ir.cost
                         << endl;
                    cerr << "Inference result's discrete cost=" << gcost_inf
                         << endl;
                }
        ...
        if (verbose) {
            for (unsigned j = 0; j < refs_in.size(); j++) {
                *costs_out << "Discrete costs of reference" << j
                           << ": total=" << std::get<0>(avg_scores[j])
                           << " average="
                           << std::get<0>(avg_scores[j]) / line_count << endl;
                *costs_out << "Continuous/Fractional costs of inference result"
                           << j << ": total=" << std::get<1>(avg_scores[j])
                           << " average="
                           << std::get<1>(avg_scores[j]) / line_count << endl;
                *costs_out << "Discrete costs of inference result " << j
                           << ": total=" << std::get<2>(avg_scores[j])
                           << " average="
                           << std::get<2>(avg_scores[j]) / line_count << endl;
            }
        }

        double elapsed = timer_dec.elapsed();
        cerr << "Relaxed optimisation decoding is finished!" << endl;
        cerr << "Decoded " << line_count << " sentences, completed in "
             << elapsed / 1000 << "(s)" << endl;
        if (timing) {
            cerr << "Decoding time info: " << endl;
            cerr << "Number of iterations=" << dti.iterations << endl;
            cerr << "Elapsed Forward=" << dti.elapsed_fwd / 1000 << "(s)"
                 << endl;
            cerr << "Elapsed Backward=" << dti.elapsed_bwd / 1000 << "(s)"
                 << endl;
            cerr << "Elapsed Update=" << dti.elapsed_upd / 1000 << "(s)"
                 << endl;
            cerr << "Elapsed Others=" << dti.elapsed_oth / 1000 << "(s)"
                 << endl;
        }
    } else {
        ...
        while (getline(*src_in, line_src) && getline(*ref_in, line_ref)) {
            ...
            RelOptOutput ir
                = relopt_decoder.GDDecodeNBest(line_src, v_kbests, relopt_cf);
            *trg_out << ir.decoded_sent << endl; // write to output(s)
