/* This is an implementation of ensemble decoder for Mantidae, adapted from
 * ensemble-decoder.{h,cc} of lamtram toolkit
 * (https://github.com/neubig/lamtram).
 * Adapted by Cong Duy Vu Hoang (duyvuleo@gmail.com,
...
public:
    EnsembleDecoderHyp(float score, const RNNPointer& state,
        const Sentence& sent, const Sentence& align)
        : score_(score)
        , state_(state)
        , sent_(sent)
        , align_(align)
...
inline bool operator<(
    const EnsembleDecoderHypPtr& lhs, const EnsembleDecoderHypPtr& rhs)
{
    if (lhs->GetScore() != rhs->GetScore())
        return lhs->GetScore() > rhs->GetScore();
    return lhs->GetSentence() < rhs->GetSentence();
    // float score_l = lhs->GetScore()/lhs->GetSentence().size(), score_r =
    // rhs->GetScore()/rhs->GetSentence().size();//score with word-based length
    // normalization
    // if( score_l != score_r) return score_l > score_r;
    // return lhs->GetSentence() < rhs->GetSentence();//shorter is better!
}

template <class AM_t> class EnsembleDecoder {

public:
    EnsembleDecoder(
        const std::vector<std::shared_ptr<AM_t> >& ams, dynet::Dict* td);
    ~EnsembleDecoder() {}

    EnsembleDecoderHypPtr Generate(
        const Sentence& sent_src, dynet::ComputationGraph& cg);
    std::vector<EnsembleDecoderHypPtr> GenerateNbest(
        const Sentence& sent_src, size_t nbest, dynet::ComputationGraph& cg);

    // Ensemble together probabilities or log probabilities for a single word
    Expression EnsembleProbs(
        const std::vector<Expression>& in, dynet::ComputationGraph& cg);
    Expression EnsembleLogProbs(
        const std::vector<Expression>& in, dynet::ComputationGraph& cg);
    ...
    void SetEnsembleOperation(const std::string& ensemble_operation)
    {
        ensemble_operation_ = ensemble_operation;
    }
...
template <class AM_t>
EnsembleDecoder<AM_t>::EnsembleDecoder(
    const std::vector<std::shared_ptr<AM_t> >& ams, dynet::Dict* td)
    : word_pen_(0.f)
    , unk_pen_(0.f)
    , size_limit_(2000)
    , beam_size_(1)
    , ensemble_operation_("sum")
    , verbose_(false)
{
    if (ams.size() == 0)
...
template <class AM_t>
Expression EnsembleDecoder<AM_t>::EnsembleProbs(
    const std::vector<Expression>& in, dynet::ComputationGraph& cg)
{
    if (in.size() == 1)
        return in[0];
...
template <class AM_t>
Expression EnsembleDecoder<AM_t>::EnsembleLogProbs(
    const std::vector<Expression>& in, dynet::ComputationGraph& cg)
{
    if (in.size() == 1)
        return in[0];
...
template <class AM_t>
EnsembleDecoderHypPtr EnsembleDecoder<AM_t>::Generate(
    const Sentence& sent_src, dynet::ComputationGraph& cg)
{
...
template <class AM_t>
std::vector<EnsembleDecoderHypPtr> EnsembleDecoder<AM_t>::GenerateNbest(
    const Sentence& sent_src,
    ...
    dynet::ComputationGraph& cg)
{
    ...
    vector<EnsembleDecoderHypPtr> curr_beam(
        1, EnsembleDecoderHypPtr(new EnsembleDecoderHyp(
               0.0, RNNPointer(-1), Sentence(1, sos_sym), Sentence(1, 0))));

    int bid;
    Expression empty_idx;

    size_limit_ = sent_src.size() * 3 /*x*/; // not generating target with
                                             // length "x times" the source
                                             // length
    ...
    for (int sent_len = 0; sent_len <= size_limit_; sent_len++) {
        ...
        vector<tuple<dynet::real, int, int, int> > next_beam_id(beam_size_ + 1,
            tuple<dynet::real, int, int, int>(-DBL_MAX, -1, -1, -1));
        ...
        for (int hypid = 0; hypid < (int)curr_beam.size(); hypid++) {
            ...
            if (sent_len != 0 && *sent.rbegin() == eos_sym)
                continue;
            ...
            for (int j : boost::irange(0, (int)ams_.size())) {
                // if (verbose_) cerr << "Forward(); state=" <<
                // curr_hyp->GetState() << endl;
                i_softmaxes.push_back(ams_[j]->Forward(sent, sent_len,
                    ensemble_operation_ == "logsum", curr_hyp->GetState(),
                    last_states[hypid], cg, i_aligns));
            }
            ...
            if (ensemble_operation_ == "sum") {
                ...
                assert(
                    string("Bad ensembling operation: " + ensemble_operation_)
                        .c_str());

            // Get the (log) softmax predictions
            // cerr << "GenerateNbest::(2)::(c,softmax) ";
            vector<dynet::real> softmax
                = as_vector(cg.incremental_forward(i_logprob));
            ...
            if (i_aligns.size() != 0) {
                ...
                vector<dynet::real> align
                    = as_vector(cg.incremental_forward(ens_align));
                best_align = 0;
            ...
            for (int wid = 0; wid < (int)softmax.size(); wid++) {
                ...
                for (bid = beam_size_;
                     bid > 0 && my_score > std::get<0>(next_beam_id[bid - 1]);
                     bid--)
                    next_beam_id[bid] = next_beam_id[bid - 1];
                next_beam_id[bid] = tuple<dynet::real, int, int, int>(
                    my_score, hypid, wid, best_align);
            }
        }
        ...
        for (int i = 0; i < beam_size_; i++) {
            ...
            // if (verbose_)
            //	cerr << "Adding " << ptd_->convert(wid) << "(" << wid << ")" <<
            //" @ beam " << i << ": score=" << std::get<0>(next_beam_id[i]) -
            //curr_beam[hypid]->GetScore() << " state=" << last_states[hypid] <<
            //endl;
            ...
            EnsembleDecoderHypPtr hyp(new EnsembleDecoderHyp(
                score, last_states[hypid], next_sent, next_align));

            if (wid == eos_sym
                && hyp->GetSentence().size()
                    == 2) // as of 26 April 2017: excluding: <s> </s>
                continue;
        ...
        if (nbest.size() != 0) {
            ...
            if (nbest.size() == nbest_size
                && (curr_beam.size() == 0
                       || (*nbest.rbegin())->GetScore()
                           >= next_beam[0]->GetScore()))
                return nbest;
        }
    ...
    if (verbose_)
        cerr << "WARNING: Generated sentence size exceeded " << size_limit_
             << ". Truncating." << endl;
