...
Expression arange(ComputationGraph& cg, unsigned begin, unsigned end,
    bool log_transform, std::vector<float>* aux_mem)
{
    aux_mem->clear();
    ...
    return input(
        cg, Dim({ end - begin }), aux_mem, dynet::default_device); // dynet v2
}
...
// Chris -- this should be a library function
Expression dither(ComputationGraph& cg, const Expression& expr, float pad_value,
    std::vector<float>* aux_mem)
{
    ...
    Expression padding = input(
        cg, Dim({ shape.cols() }), aux_mem, dynet::default_device); // dynet v2
    Expression padded
        = concatenate(std::vector<Expression>({ padding, expr, padding }));
    ...
    return concatenate_cols(
        std::vector<Expression>({ left_shift, expr, right_shift }));
}
...
{
    return min(rectify(expr - (value - epsilon)),
               rectify(-expr + (value + epsilon)))
        / epsilon;
}

Expression geq(
    const Expression& expr, float value, Expression& one, float epsilon = 0.01)
{
