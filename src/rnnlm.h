 ...
 * \brief This structure wraps any RNN to train a language model with
 * minibatching
 * \details Recurrent neural network based language modelling maximizes the
 * likelihood
 * of a sentence \f$\textbf s=(w_1,\dots,w_n)\f$ by modelling it as :
 *
 * \f$L(\textbf s)=p(w_1,\dots,w_n)=\prod_{i=1}^n p(w_i\vert
 * w_1,\dots,w_{i-1})\f$
 *
 * Where \f$p(w_i\vert w_1,\dots,w_{i-1})\f$ is given by the output of the RNN
 * at step \f$i\f$
 *
 * In the case of training with minibatching, the sentences must be of the same
 * length in
 * each minibatch. This requires some preprocessing (see `train_rnnlm-batch.cc`
 * for example).
 *
 * Reference : [Mikolov et al.,
...
template <class Builder> struct RNNLanguageModel {
...
protected:
    ...
    bool reverse_
        = false; // left-to-right (false) or right-to-left (true) direction
    float dropout_p = 0.f;
    ...
    Expression
        i_We_; // storing word embedding matrix of all words in the vocabulary
...
public:
    ...
    */
    ...
    explicit RNNLanguageModel(ParameterCollection* model, unsigned layers,
        unsigned input_dim, unsigned hidden_dim, unsigned vocab_size,
        bool reverse)
        : layers_(layers)
        ...
        , reverse_(reverse)
    {
        ...
        rnn_
            = BuilderPtr(new Builder(layers_, input_dim_, hidden_dim_, *model));
        pmodel_ = model;
    }

    void CreateModel(ParameterCollection* model, unsigned layers,
        unsigned input_dim, unsigned hidden_dim, unsigned vocab_size,
        bool reverse)
    {
        layers_ = layers;
    ...
    void LoadModel(ParameterCollection* model, const std::string& model_file)
    {
        ...
        ia >> layers_ >> input_dim_ >> hidden_dim_ >> vocab_size_ >> dropout_p
            >> reverse_;
        ...
        dynet::load_dynet_model(model_file, pmodel_); // FIXME: use binary
    ...
    void SaveModel(const std::string& model_file)
    {
        ...
        oa << layers_ << input_dim_ << hidden_dim_ << vocab_size_ << dropout_p
           << reverse_;

        // dynet v2
        dynet::save_dynet_model(model_file, pmodel_); // FIXME: use binary
                                                      // streaming instead for
                                                      // saving disk spaces
    }

    void DisableDropout() { rnn_->disable_dropout(); }

    void EnableDropout(float p) { rnn_->set_dropout(p); }
    ...
    * \param bsize Batch size (`id` + `bsize` should be smaller than the size of
    * the dataset)
    * \param tokens Number of tokens processed by the model (used for loos per
    * token computation)
    ...
    */
    Expression BuildLMGraph(const vector<vector<int> >& sents,
        ...
        unsigned bsize /*mini-batch size*/, unsigned& tokens /*token count*/,
        unsigned& unk_tokens /*<unk> token count*/, ComputationGraph& cg)
    {
        ...
        for (unsigned t = 1; t < slen; ++t) {
            auto ct = t;
            if (reverse_ == true)
                ct = slen - t - 1; // right-to-left direction
            ...
            for (unsigned i = 0; i < bsize; ++i) {
                ...
                if (next_arr[i] != (unsigned)*sents[sid].rbegin())
                    tokens++;
                if (next_arr[i] == (unsigned)kUNK)
                    unk_tokens++;
            }
        ...
        unsigned& unk_tokens /*<unk> token count*/, ComputationGraph& cg)
    {
        ...
        for (unsigned t = 0; t < len; ++t) {
            auto ct = t;
            if (reverse_ == true)
                ct = len - t - 1; // right-to-left direction

            for (size_t bs = 0; bs < sents.size(); bs++) {
                words[bs] = (sents[bs].size() > t)
                    ? ((reverse_ == true && t != 0 /*BOS*/)
                              ? (unsigned)sents[bs][ct + 1]
                              : (unsigned)sents[bs][t])
                    : kEOS;
                next_words[bs] = (sents[bs].size() > (t + 1))
                    ? ((reverse_ == true && ct != 0 /*EOS*/)
                              ? (unsigned)sents[bs][ct]
                              : (unsigned)sents[bs][t + 1])
                    : kEOS;
                if (sents[bs].size() > t) {
                    tokens++;
                    if (sents[bs][t] == kUNK)
    ...
    Expression ComputeNLL(const vector<int>& sent, ComputationGraph& cg)
    {
        const unsigned slen = sent.size();
        std::vector<int> rsent = sent;
        if (reverse_ == true)
    ...
    {
        // expected embedding
        return (i_We_ /*hidden_dim x vocab_size_*/
            * i_y /*vocab_size_ x 1*/); // hidden_dim x 1
    }

    Expression BuildRelOptGraph(size_t algo,
        std::vector<dynet::Parameter>& v_params, ComputationGraph& cg,
        dynet::Dict& d)
    {
        // Initialize the RNN for a new computation graph
        ...
        int slen = v_params.size(); // desired target length (excluding BOS and
                                    // EOS tokens)
        ...
        for (auto t = 0; t < slen + 1; ++t) {
            ...
            Expression i_r_t = affine_transform(
                { i_bias, i_R, i_y_t }); // i_bias + i_R * i_y_t;
    ...
    * \f$\frac{e^{\frac{r_t^{(i)}}{T}}}{\sum_{j=1}^{\vert
    * V\vert}e^{\frac{r_t^{(j)}}{T}}}\f$).
    *  Intuitively lower temperature -> less deviation from the distribution (=
    * more "standard" samples)
    */
    void RandomSample(dynet::Dict& d, int max_len = 150, float temp = 1.0)
    {
        ...
        while (len < max_len) {
            ...
            if (w == kEOS) {
                // If the sampled token is an EOS, reinitialize network and
