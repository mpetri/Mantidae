...
void TrainModel(ParameterCollection& model, RNNLM_t& rnn, Corpus& traincor,
    Corpus& devcor, unsigned patience, unsigned eta_patience, Trainer* sgd,
    const string& param_file, float dropout_p);
template <class RNNLM_t>
void TrainModel_Batch(ParameterCollection& model, RNNLM_t& rnn,
    Corpus& traincor, Corpus& devcor, unsigned patience, unsigned eta_patience,
    Trainer* sgd, const string& param_file, float dropout_p);

template <class RNNLM_t>
void TestModel(ParameterCollection& model, RNNLM_t& rnn, const Corpus& testcor);

template <class Builder> int main_body(variables_map vm);
...
int main(int argc, char** argv)
{
    ...
    opts.add_options()("help", "print help message")("train,t", value<string>(),
        "file containing training sentences")("dev,d", value<string>(),
        "file containing development sentences")("test,T", value<string>(),
        "file containing testing source sentences for computing perplexity "
        "scores")("lang", value<bool>()->default_value(true),
        "use source or target language from a corpus, e.g., source ||| target")(
        "initialise,l", value<string>(), "load initial parameters from file")(
        "parameters,s", value<string>(), "save best parameters to this file")(
        "layers", value<unsigned>()->default_value(LAYERS),
        "use <num> layers for RNN components")("hidden_dim,h",
        value<unsigned>()->default_value(HIDDEN_DIM),
        "use <num> dimensions for recurrent hidden states")("input_dim,i",
        value<unsigned>()->default_value(INPUT_DIM),
        "use <num> dimensions for input embedding")("batch_size,b",
        value<unsigned>()->default_value(BATCH_SIZE),
        "impose mini-batch size for training")("gru",
        "use Gated Recurrent Unit (GRU) for recurrent structure; default RNN")(
        "lstm", "use Long Short Term Memory (LSTM) for recurrent structure; "
                "default RNN")("dglstm", "use Depth-Gated Long Short Term "
                                         "Memory (DGLSTM) for recurrent "
                                         "structure; default RNN")("dropout",
        value<float>(), "apply dropout technique (Gal et al., 2015) for RNN")(
        "sgd_trainer", value<unsigned>()->default_value(0),
        "use specific SGD trainer (0: vanilla SGD; 1: momentum SGD; 2: "
        "Adagrad; 3: AdaDelta; 4: Adam)")("sparse_updates",
        value<bool>()->default_value(true),
        "enable/disable sparse update(s) for lookup parameter(s)")("r2l_target",
        "use right-to-left direction for target during training; default not")(
        "max_loss", value<double>()->default_value(MAX_LOSS),
        "impose max loss if possible (for interruped running jobs)")("treport",
        value<unsigned>()->default_value(TREPORT),
        "impose instance reporting value (during training); default 50")(
        "dreport", value<unsigned>()->default_value(DREPORT),
        "impose instance reporting value (during training) on development/test "
        "sets; default 500")("eta_epoch",
        value<unsigned>()->default_value(ETA_EPOCH),
        "limit number of epochs for scheduling the learning rate, e.g., "
        "decreasing by a factor of eta_decay; default 4")("max_epoch",
        value<unsigned>()->default_value(MAX_EPOCH),
        "limit number of epochs for training; default 20")("patience",
        value<unsigned>()->default_value(0),
        "no. of times in which the model has not been improved for early "
        "stopping; default none")("eta_patience",
        value<unsigned>()->default_value(0),
        "no. of times in which the model has not been improved, e.g., for "
        "starting learning rate annealing (e.g., halving)")("lambda",
        value<dynet::real>()->default_value(1e-6),
        "the L2 regularization coefficient; default 1e-6.")("eta",
        value<dynet::real>()->default_value(ETA),
        "the learning rate of neural network model; default 0.1")("eta_decay",
        value<dynet::real>()->default_value(ETA_DECAY),
        "decay value for decreasing the learning rate; default 2.")("seed",
        value<unsigned>()->default_value(1),
        "value for random initialization; default 1")(
    ...
    if (vm.count("help") || vm.count("train") != 1
        || (vm.count("devel") != 1 && !(vm.count("test") == 0))) {
        cout << opts << "\n";
        return 1;
    }
...
template <class Builder> int main_body(variables_map vm)
{
    ...
    if (vm.count("dropout")) // apply dropout technique
    {
        ...
        cerr << "Applying dropout technique with dropout probability p="
             << dropout_p << "..." << endl;
    }
    ...
    if (vm.count("parameters"))
        ...
        ostringstream os;
        os << "rnnlm" << '_' << LAYERS << '_' << HIDDEN_DIM << '_' << INPUT_DIM
           << '_' << VOCAB_SIZE << "_pid" << getpid() << ".params";
        param_file = os.str();
    }
    ...
    if (vm.count("r2l_target"))
        cerr << "Using right-to-left direction..." << endl;
    ...
    if (vm.count("initialise")) // load from pre-trained model
        ...
        lm.CreateModel(&model, LAYERS, INPUT_DIM, HIDDEN_DIM, VOCAB_SIZE,
            vm.count("r2l_target"));
    ...
    if (vm.count("test")) {
        ...
        TrainModel_Batch(model, lm, traincor, devcor,
            vm["patience"].as<unsigned>(), vm["eta_patience"].as<unsigned>(),
            sgd_trainer, param_file, dropout_p);
...
void TestModel(ParameterCollection& model, RNNLM_t& rnn, const Corpus& testcor)
{
    ...
    cerr << "------------------------------------------------------------------"
         ...
         << endl;
    cerr << "***TEST "
         << "sentences=" << testcor.size() << " unks=" << unk_dtokens
         << " E=" << (dloss / dtokens) << " ppl=" << exp(dloss / dtokens)
         << ' ';
    cerr << "\n----------------------------------------------------------------"
            "---------\n"
         << endl;
...
template <class RNNLM_t>
void TrainModel(ParameterCollection& model, RNNLM_t& rnn, Corpus& traincor,
    Corpus& devcor, unsigned patience, unsigned eta_patience,
    Trainer* sgd_trainer, const string& param_file, float dropout_p)
{
    ...
    while (epoch < MAX_EPOCH) {
        ...
        for (unsigned i = 0; i < report_every_i; ++i, ++si) {
            if (si == order.size()) {
                ...
                if (ETA_EPOCH > 0 && epoch >= ETA_EPOCH)
                    sgd_trainer->learning_rate
                        /= ETA_DECAY; // @vhoang2: learning rate annealing
                                      // (after lr_epochs, for every next epoch,
                                      // the learning rate will be decreased by
                                      // a factor of eta_decay.
            ...
            ComputationGraph cg;
            unsigned c1 = 0, c2 = 0;
            Expression i_xent = rnn.BuildLMGraph(
                traincor[order[si]], c1 /*tokens*/, c2 /*unk_tokens*/, cg);
        ...
        sgd_trainer->status();
        cerr << "sents=" << lines << " unks=" << unk_tokens
             << " E=" << (loss / tokens) << " ppl=" << exp(loss / tokens)
             << ' ';
        double elapsed = timer_iteration.elapsed();
        cerr << "[time_elapsed=" << elapsed << "(msec)"
             << " (" << tokens * 1000.f / elapsed << " words/sec)]" << endl;
        timer_iteration.reset();

        // rnn.RandomSample(); // uncomment this to see some randomly-generated
        // sentences
        ...
        if (report % devcor_every_i_reports == 0) {
            ...
            for (auto& sent : devcor) {
                ComputationGraph cg;
                Expression i_xent
                    = rnn.BuildLMGraph(sent, dtokens, unk_dtokens, cg);
            ...
            cerr << "\n***DEV [epoch=" << (lines / (double)traincor.size())
                 << " eta=" << sgd_trainer->learning_rate << "]"
                 << " sents=" << devcor.size() << " unks=" << unk_dtokens
                 << " E=" << (dloss / dtokens)
            ...
            if (eta_patience > 0 && cpt > 0 && cpt % eta_patience == 0) {
                cerr << "The model has not been improved for " << eta_patience
...
struct SingleLength {
    SingleLength(const vector<Sentence>& v)
        : vec(v)
    {
    }
...
inline size_t CalcSize(const Sentence& src, int trg) { return src.size() + 1; }
...
{
    if (BATCH_SIZE == 1) {
        TrainModel(model, rnn, traincor, devcor, patience, eta_patience,
            sgd_trainer, param_file, dropout_p);
        return;
    }
    ...
    double best = 9e+99;
    unsigned si = 0, last_print = 0, lines = 0, epoch = 0,
             cpt = 0 /*count of patience*/;
    MyTimer timer_epoch("completed in"), timer_iteration("completed in");
    while (epoch < MAX_EPOCH) {
        rnn.EnableDropout(dropout_p);
        ...
        for (unsigned i = 0; i < dev_every_i_reports; ++si) {
            if (si == train_ids_minibatch.size()) {
                ...
                cerr << "**SHUFFLE\n";
                shuffle(train_ids_minibatch.begin(), train_ids_minibatch.end(),
                    *dynet::rndeng);
            ...
            Expression i_xent
                = rnn.BuildLMGraph(train_minibatch[train_ids_minibatch[si]],
                    c1 /*tokens*/, c2 /*unk_tokens*/, cg);
            ...
            if (lines / report_every_i != last_print
                ...
                double elapsed = timer_iteration.elapsed();
                cerr << "[time_elapsed=" << elapsed << "(msec)"
                     << " (" << tokens * 1000.f / elapsed << " words/sec)]"
                     << endl;
            }
        }
        ...
        cerr << "--------------------------------------------------------------"
                "------------------------------------------"
             << endl;
        cerr << "***DEV [epoch=" << epoch + lines / (double)traincor.size()
             << " eta=" << sgd_trainer->learning_rate << "]"
             << " sents=" << devcor.size() << " unks=" << unk_dtokens
             << " E=" << (dloss / dtokens) << " ppl=" << exp(dloss / dtokens)
             << ' ';
...
Corpus Read_Corpus(const string& fp, bool dir)
{
    ...
    while (std::getline(f, line)) {
        ...
        if ((source.front() != kSOS && source.back() != kEOS)) {
            cerr << "Sentence in " << fp << ":" << lc
                 << " didn't start or end with <s> and </s>\n";
            abort();
        }
