...
void Dual_Learn(ParameterCollection& mod_am_s2t, AM_t& p_am_s2t,
    ParameterCollection& mod_am_t2s, AM_t& p_am_t2s,
    ParameterCollection& mod_mlm_s, LM_t& p_mlm_s,
    ParameterCollection& mod_mlm_t, LM_t& p_mlm_t, const MonoCorpus& mono_cor_s,
    const MonoCorpus& mono_cor_t, const ParaCorpus& dev_cor, unsigned K,
    unsigned beam_size, float alpha, float gamma_1, float gamma_2,
    unsigned opt_type, const string& modfile_am_s2t,
    const string& modfile_am_t2s);
...
int main(int argc, char** argv)
{
    ...
    opts.add_options()("help", "print help message")("config,c",
        ...
        ("train,t", value<string>(),
            "file containing training parallel sentences (for source/targer "
            "vocabulary building on-the-fly), with "
            "each line consisting of source ||| target.")("dev,d",
            value<string>(),
            "file containing development parallel sentences, with "
            "each line consisting of source ||| target.")("initialise_am_s2t",
            value<string>(),
            "load pre-trained model parameters (source-to-target AM model) "
            "from file")("initialise_am_t2s", value<string>(),
            "load pre-trained model parameters (target-to-source AM model) "
            "from file")("initialise_rnnlm_s", value<string>(),
            "load pre-trained model parameters (source RNNLM model) from file")(
            "initialise_rnnlm_t", value<string>(),
            "load pre-trained model parameters (target RNNLM model) from file")
        //-----------------------------------------
        ("save_am_s2t", value<string>(), "save learned model parameters "
                                         "(source-to-target AM model) to file")(
            "saves_am_t2s", value<string>(),
            "save learned model parameters (target-to-source AM model) to file")
        //-----------------------------------------
        ("sgd_trainer", value<unsigned>()->default_value(0),
            "use specific SGD trainer (0: vanilla SGD; 1: momentum SGD; 2: "
            "Adagrad; 3: AdaDelta; 4: Adam; 5: RMSProp)")
        //-----------------------------------------
        ("model_conf", value<string>(),
            "config file specifying model configurations for all models (line "
            "by line)")("K", value<unsigned>()->default_value(10),
            "the K value for sampling K-best translations from AM models")(
            "beam_size", value<unsigned>()->default_value(5),
            "the beam size of beam search decoding")("alpha",
            value<float>()->default_value(0.5f),
            "the alpha hyper-parameter for balancing the rewards")("gamma_1",
            value<float>()->default_value(0.1f),
            "the gamma 1 hyper-parameter for stochastic gradient update in "
            "tuning source-to-target AM model")("gamma_2",
            value<float>()->default_value(0.1f),
            "the gamma 2 hyper-parameter for stochastic gradient update in "
            "tuning target-to-source AM model")
        //-----------------------------------------
        ("mono_s", value<string>()->default_value(""),
            "File to read the monolingual source from")("mono_t",
            value<string>()->default_value(""),
            "File to read the monolingual target from")
        //-----------------------------------------
        ("epoch,e", value<unsigned>()->default_value(10),
            "number of training epochs, 10 by default")("dev_round",
            value<unsigned>()->default_value(25000),
            "number of rounds for evaluating over development data, 25000 by "
            "default")
        //-----------------------------------------
    ...
    // monolingual corpora
    // Assume that these monolingual corpora use the same vocabularies with
    // parallel corpus
    // FIXME: otherwise?
    ...
    std::shared_ptr<AttentionalModel<LSTMBuilder> > p_am_s2t,
        p_am_t2s; // FIXME: assume all models use the same LSTM RNN structure
                  // (for quick implementation)
    std::shared_ptr<RNNLanguageModel<LSTMBuilder> > p_mlm_s,
        p_mlm_t; // FIXME: how to support n-gram LM model (e.g., KenLM?) as
                 // well???
    ...
    p_am_s2t.reset(new AttentionalModel<LSTMBuilder>(&model_am_s2t,
        SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, SLAYERS, TLAYERS, HIDDEN_DIM, ALIGN_DIM,
        true, false, false, false, false, false)); // standard model for now
    ...
    ss.str(line);
    ss >> SLAYERS >> TLAYERS >> HIDDEN_DIM >> ALIGN_DIM;
    p_am_t2s.reset(new AttentionalModel<LSTMBuilder>(&model_am_t2s,
        SRC_VOCAB_SIZE, TGT_VOCAB_SIZE, SLAYERS, TLAYERS, HIDDEN_DIM, ALIGN_DIM,
    ...
    p_mlm_s.reset(new RNNLanguageModel<LSTMBuilder>(
        &model_mlm_s, SLAYERS, HIDDEN_DIM, HIDDEN_DIM, SRC_VOCAB_SIZE, false));
    ...
    p_mlm_t.reset(new RNNLanguageModel<LSTMBuilder>(
        &model_mlm_t, TLAYERS, HIDDEN_DIM, HIDDEN_DIM, TGT_VOCAB_SIZE, false));
    InitialiseModel(model_mlm_t, vm["initialise_rnnlm_t"].as<string>());

    //--- execute dual-learning
    Dual_Learn<AttentionalModel<LSTMBuilder>, RNNLanguageModel<LSTMBuilder> >(
        model_am_s2t, *p_am_s2t, model_am_t2s, *p_am_t2s, model_mlm_s, *p_mlm_s,
        model_mlm_t, *p_mlm_t, mono_cor_s, mono_cor_t, dev_cor, K, beam_size,
        alpha, gamma_1, gamma_2, vm["sgd_trainer"].as<unsigned>(),
...
void Dual_Learn(ParameterCollection& mod_am_s2t, AM_t& am_s2t,
    ParameterCollection& mod_am_t2s, AM_t& am_t2s,
    ParameterCollection& mod_mlm_s, LM_t& mlm_s, ParameterCollection& mod_mlm_t,
    LM_t& mlm_t, const MonoCorpus& mono_cor_s, const MonoCorpus& mono_cor_t,
    const ParaCorpus& dev_cor, unsigned K, unsigned beam_size, float alpha,
    float gamma_1, float gamma_2, unsigned opt_type,
    const string& modfile_am_s2t, const string& modfile_am_t2s)
{
    // set up the decoder(s)
    EnsembleDecoder<AM_t> edec_s2t(
        std::vector<std::shared_ptr<AM_t> >({ std::make_shared<AM_t>(am_s2t) }),
        &tdict);
    edec_s2t.SetBeamSize(beam_size);
    EnsembleDecoder<AM_t> edec_t2s(
        std::vector<std::shared_ptr<AM_t> >({ std::make_shared<AM_t>(am_t2s) }),
        &tdict);
    ...
    if (opt_type == 1) {
        ...
        assert("Unknown SGD trainer type! (0: vanilla SGD; 1: momentum SGD; 2: "
               "Adagrad; 3: AdaDelta; 4: Adam; 5: RMSProp)");
        ...
        || epoch_t2s < MAX_EPOCH) // FIXME: simple stopping criterion, another?
    {
        ...
        if (id_s == orders_s.size()) {
            shuffle(
                orders_s.begin(), orders_s.end(), *rndeng); // to make it random
            id_s = 0; // reset id
            epoch_s2t++; // FIXME: adjust the learning rate if required?
        }
        if (id_t == orders_t.size()) {
            shuffle(
                orders_t.begin(), orders_t.end(), *rndeng); // to make it random
            id_t = 0; // reset id
            epoch_t2s++; // FIXME: adjust the learning rate if required?
        }

        // sample sentence sentA and sentB from mono_cor_s and mono_cor_s
        // respectively
        Sentence sent;
        ...
        // generate K translated sentences s_{mid,1},...,s_{mid,K} using beam
        // search according to translation model P(.|sentA; mod_am_s2t).
        std::vector<EnsembleDecoderHypPtr> v_mid_hyps
            = p_edec->GenerateNbest(sent, K, cg);
        ...
        for (auto& mid_hyp : v_mid_hyps) {
            ...
            // const Sentence& mid_aligns = mid_hyp->GetAlignment();//unused for
            // now

            // set the language-model reward for current sampled sentence from
            // p_mlm_t
            auto r1
                = -p_mlm->ComputeNLL(mid_trans, cg) /*negative log likelihood*/
                / mid_trans.size() /*length-normalized*/; // FIXME: negative
                                                          // entropy as reward,
                                                          // correct?

            // set the communication reward for current sampled sentence from
            // p_am_t2s
            auto r2 = -p_am_t2s->BuildGraph(mid_trans, sent, cg, stats, nullptr,
                nullptr); // = log(P(sentA|trans; mod_am_t2s))

            auto r = alpha * r1 + (1.f - alpha) * r2; // reward interpolation
            v_r1.push_back(
                r * (-p_am_s2t->BuildGraph(sent, mid_trans, cg, stats, nullptr,
                        nullptr)) /*log(P(trans|sentA; mod_am_s2t))*/);
        ...
        // testing over the development data to check the improvements (after a
        ...
        if (r == DEV_ROUND) {
            ...
            for (unsigned i = 0; i < dev_cor.size(); ++i) {
                ...
                auto i_xent_s2t = am_s2t.BuildGraph(ssent, tsent, cg,
                    dstats_s2t, nullptr, nullptr, nullptr, nullptr);
                auto i_xent_t2s = am_t2s.BuildGraph(tsent, ssent, cg,
                    dstats_t2s, nullptr, nullptr, nullptr, nullptr);
            ...
            cerr << "----------------------------------------------------------"
                 ...
                 << endl;
            cerr << "***DEV (s2t) [epoch="
                 << epoch_s2t + (float)id_s / (float)orders_s.size()
                 << " eta=" << p_sgd_s2t->learning_rate << "]"
                 << " sents=" << dev_cor.size()
                 << " src_unks=" << dstats_s2t.words_src_unk
                 << " trg_unks=" << dstats_s2t.words_tgt_unk
                 << " E=" << (dstats_s2t.loss / dstats_s2t.words_tgt)
                 << " ppl=" << exp(dstats_s2t.loss / dstats_s2t.words_tgt)
                 << endl;
            cerr << "***DEV (t2s) [epoch="
                 << epoch_t2s + (float)id_t / (float)orders_t.size()
                 << " eta=" << p_sgd_t2s->learning_rate << "]"
                 << " sents=" << dev_cor.size()
                 << " src_unks=" << dstats_t2s.words_src_unk
                 << " trg_unks=" << dstats_t2s.words_tgt_unk
                 << " E=" << (dstats_t2s.loss / dstats_t2s.words_tgt)
                 << " ppl=" << exp(dstats_t2s.loss / dstats_t2s.words_tgt)
                 << endl;
            cerr << "----------------------------------------------------------"
                    "----------------------------------------------"
                 << endl;
